...
Usage:  python validate_design.py design.json design-schema.v5.json [-v]
"""
import json
import math
import sys
...
def _seg_cross(a, b, c, d, tol=1e-4):
    ...
    def o(p, q, r):
        L = math.hypot(q[0] - p[0], q[1] - p[1]) or 1.0
        return ((q[0] - p[0]) * (r[1] - p[1]) - (q[1] - p[1]) * (r[0] - p[0])) / L
...
if __name__ == "__main__":
    ...
    serr = sorted(jsonschema.Draft202012Validator(sch).iter_errors(doc),
    ...
    sys.exit(1 if (serr or ierr) else 0)
