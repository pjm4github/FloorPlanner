...
| A2 ⏸⏬ | **DROPPED BEHIND [D68](defects/0068-the-3d-view-renders-every-level-not-the.md) — 2026‑08‑11.** *If the viewer renders the ACTIVE FLOOR ONLY, D11's z collapse stops mattering for the common case: one floor at one height is correct by definition.* So **floor-scoping the viewer is a cheaper route to a usable 3D view than fixing the z stack**, and A2 goes behind it rather than ahead. Recorded here so the reprioritisation is on disk rather than in a conversation. **D11 — the runtime z collapse. THE HANG IS PARKED AS NOT REPRODUCIBLE (2026‑08‑09).** Measured: five orders of magnitude on either z step changes no event count and the named test passes in ~0.3 s. Ruling 4's scheme and the four competing systems stay open; the instrument is kept at `evidence/d11_a2_z_step_counter.py` | rule fully specified (§2 of the snapshot). **Instrument the hang with a bounded event counter first — do not choose constants to avoid a symptom** | bring-to-front sticks; ghost floors stay behind the active floor; multi-floor banding unchanged |
