...
| [`0012-readback-prism-outlines.md`](0012-readback-prism-outlines.md) | **Read-back, measurement only — ruling 0010 item ONE's opening question.** Of the 28 fallback items, **19 have a filled BODY prism can extrude, 6 give a PARTIAL solid, 3 have NONE** (prism would be worse than the box). **The split is by FORM and it is stark**: the four furniture forms are **16 BODY of 18**, `vehicle` is **3 of 10** — so **prism may retire four of the five pending generators** and vehicle-first survives for a stronger reason than item count. **The instrument's first cut was wrong and is withdrawn**: one 25% threshold split `lawnmower` from `snowblower`, two structurally identical symbols, and it was caught by inspecting either side of the line rather than by reading the totals. |
