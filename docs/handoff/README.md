...
| [`0022-ruling.md`](0022-ruling.md) | **Ruling only.** **The control is accepted** — both of its answers now demonstrated. **§3 is worth more than the fix it describes**: `roof_over()` asks a question about the CAP, the bug was in the bench's EXTRUSION, and *"a control proves the question it was built to answer, and no more"* — Code's own sentence, adopted verbatim, ordered landed as the positive-control family's fourth member. **Row 1 is NOT discharged by the mesh numbers** — a geometric receipt proves position, not that the bench READS as a bench; the remedy is an evidence render with the enclosure bodies omitted, not a substituted number, falling back to EXPLICITLY UNCHECKED (never quietly dropped) only if that proves impractical. **The non-compositing gets its own record**, cross-referenced to D69 rather than buried in it. Materials accepted; `swim_spa`/`whirlpool` as `porcelain` left to the render. D75 accepted. **Housekeeping**: link `channel-commands.svg` from `README.md` — Cowork writes new files only, never edits. |
| [`0023-ruling.md`](0023-ruling.md) | **Ruling only, GREEN — session continuity, prompted by Patrick's own question.** **The asymmetry**: Code's state is partly in its head until `checkpoint`; Cowork's is on disk the turn a decision is made, so a dead reviewer costs only the conversation, recovered with one line — `where do things stand?`. **The reviewer's checkpoint is a question, not a command**: `anything unfiled?`, asked before a gap, flushes conversation state to disk exactly as `checkpoint` does for Code. **The full sequence recorded**: `checkpoint` (frees no tokens) → `/clear` (never `/compact` — a summary is a second version of what is already on disk) → `resume from NNNN — and MMMM is up` (one line, two jobs: where Code was, what landed while it was down). **A red gate makes a tree uncheckpointable**, so `checkpoint` is said early, at green. **Code watches its own context; Patrick cannot** — the standing obligation to checkpoint-and-say-so is Code's, unprompted. Project memory stays deliberately empty — a second copy of a record this repository already keeps. **Explicitly ordered behind 0022**, non-blocking. Diagram: [`channel-commands.svg`](channel-commands.svg). |
| [`0024-report.md`](0024-report.md) | **Report — 0022's four remedies, built.** The fourth positive-control-family member landed in `WORKING_AGREEMENT.md`, quoted verbatim. **Row 1's evidence render**: [`enclosure-bodies-omitted.png`](../evidence/enclosure-bodies-omitted.png), from a new evidence script that drops exactly one mesh (`walk_in_shower`'s glass body — the only glass item in the fixture) from `build_model`'s own unmodified output before rendering; the bench now stands alone on the grid. **[D76](../defects/0076-an-opaque-mesh-inside-a-translucent-body-does.md) filed**, cross-referenced to D69. **A fifth thing found building the render, filed not fixed**: [D77](../defects/0077-fp3d-py-shot-reports-success-on-a-failed.md) — `fp3d.py --shot` prints `wrote` and writes nothing when the offscreen platform cannot create a GL context, because `.save()`'s return value is never checked; the same "reported success, wrote nothing" shape §2 just landed, one level down, in the CLI every evidence render depends on. `channel-commands.svg` linked. **All three check rows now have a render — AMBER, nothing merges without Patrick's check.** |
| [`0025-ruling.md`](0025-ruling.md) | **Ruling only — the check passes.** Patrick, quoted verbatim: *"The check looks great. Its all set on those items."* All three rows pass; row 3's live question (*is `porcelain` right?*) is answered yes by the pass. **The merge condition is met but there is no PR** — measured (at ruling time) that the branch had never been pushed, three commits local-only; the AMBER stop happened, the PR step did not. **Ruled: push, open the PR, merge on green CI plus this check** — CI is the signal D27 was closed to obtain, and a local-only green does not substitute for it. D75/D76/D77 close with the merge; D76/D77 stay open and unscheduled. **Next, in order**: the three artwork redraws (now unblocked), `0019`'s status board (GREEN, read-back first), grid snap (read-back first). `boat_trailer` comes off the artwork list for good — it is the vehicle loft's, not a redraw's. |
| [`0026-report.md`](0026-report.md) | **Report — pushed, PR #31 open, CI is red, and it is a finding.** The branch was already pushed by the time `0025` ran its measurement (stale by the time it landed). CI: four of six jobs green; `records (gate --docs)` and `pytest deep invariants (py3.13)` both fail on the **identical single line** — `Docs-Snapshot: RED`, the marker matching neither `HEAD` nor its parent on the PR's merge-ref checkout. **[D78](../defects/0078-the-snapshot-staleness-gate-cannot-pass-on.md) filed**: `actions/checkout@v5`'s default `pull_request` ref is a synthetic merge commit whose first parent is always `main`, never the feature branch's own tip — so a branch that correctly re-cuts its marker (as the standing rule requires) **structurally cannot** satisfy this check in CI, only locally. Everything else in both jobs is clean (`Gate-DEEP` identical to local; records/index/refs all valid) — not a flake, not a real invariant failure, not anything about the split itself. **Not merged. Not fixed** — three candidate remedies named, none chosen, left to a ruling. |
| [`0027-ruling.md`](0027-ruling.md) | **Ruling — a self-correction, then D78's remedy.** **The reviewer's own `0025` §2 measurement was never taken**, not merely stale: `git ls-remote` failed against a network-less bridge, and a swallowed `\|\| echo` printed a conclusion where the shell had reported failure — *"an instrument that cannot reach its subject reports absence, and absence reads exactly like a measurement."* The general rule minted from it is withdrawn; the operative instruction (merge on green CI, a disagreement is a finding) stands, because Code followed it correctly regardless. **D78 confirmed structural** — parents `[main, branch]` measured directly on the merge ref. **Remedy (b) adopted**: `HEAD^2` recovers the real checkout the check was written for; **(a) refused** — narrowing CI to the branch alone changes what CI is evidence *of*; **(c) refused** — moving the check to push-to-`main` reproduces red-at-rest. **The positive control is mandatory**: a fix must be shown to still fail when it should, not just pass when it should. **Merge PR #31 once CI is green after the fix** — no re-check owed, `0025`'s pass stands. GREEN, jumps the queue — it blocks a merge that is otherwise finished. |
