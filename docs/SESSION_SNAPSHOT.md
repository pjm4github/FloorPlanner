...
| | |
|---|---|
| **`main`** | **`a598909`** at this file's cut. **`0098`-`0106` (wall id/coordinate work, per-task progress files, the ON/DEEP CI measurement) are DONE** — full trail: `handoff/README.md`'s pair table. **`0107` landed and is DONE**: `--docs` now runs LIVE in the push hook (it writes no result file, so nothing else could check it — `0105` had claimed the commit hook already covered this; it did not, and the gap cost 7 real `Docs-Refs` catches, measured in `0106-report.md`); the CI matrix simplified to one hard-coded `py3.13` leg (the prior "disabled, not deleted" matrix leg span up a runner, skipped every step, and reported success having tested nothing); `tools/gate.py`'s full mode is now OFF+ON locally, OFF+ON+DEEP only under CI (`CI=true`), extracted into a plain testable `_select_modes()`. **`0108`-`0110` spec three new AMBER features** (all queued behind PR #37/#38, per `0111`): "Snap to Grid" (per-wall, manual, AMBER), "Snap to Grid Orthogonal" (the safer variant, build **first**), a disabled placeholder menu item for the 15°-angle case (GREEN, no arithmetic exists for it — `tan 15°` is irrational, proven in `0110` §3). **`0111`/`0112` order the close-out**: three branches need a check, not two (`wall-report-id-fix` needs one too, `0102`'s AMBER tier) — land `0107`-`0112` on `main`, `0112`'s `plans.pdf` gitignore + delete (the fix-the-test half of `0111` §2 was withdrawn, the test already passes `-o tmp_path`), then bring `main` into all three branches, re-gate each, push, and stop — nothing else opens a branch until those three merge. **Numbering collisions, on the record:** `0036`, `0043`, `0050`, `0101`; neither renamed, numbering continues forward each time. |
