...
**Re-cut 2026‑08‑08, on branch `a1-d47-fragment-extract` @ `9b4f844`, with `main` @ `a1172be`.** **PHASE 4 IS COMPLETE; the GREEN batch has merged; the AMBER queue has started and A1 is at its pull request.** This file exists so a fresh session can start from disk instead of from a chat summary. It is an **index and a state marker, not a second copy of the record** — where it points at another document, that document is authoritative and this one must not be trusted over it.
...
| | |
|---|---|
| **Branch** | `a1-d47-fragment-extract` @ `9b4f844`, pushed and level with `origin`. **PR #17 is OPEN and this is where the work stops.** |
...
| **PR #17 — A1 (D47)** | **OPEN, AMBER, `mergeStateStatus: CLEAN`.** CI green on **all six jobs** (ruff · records · py3.10 · py3.13 · **windows** · deep invariants), run `31267752756`. **Its merge condition is Patrick's six-item manual check**, scripted in the PR body rather than left to rediscovery. |
...
| **Records** | **53 records — 13 open, 40 closed.** `python tools/gate.py --docs` **GREEN** (front matter valid, `INDEX.md` matches a regeneration, every defect reference resolves, migration dry run clean). `Ref-Audit: unresolved=10` — dangling *links and paths*, which are reported and never enforced; dangling *keys* would fail. |
...
- **The census reads the WORKING TREE (D51).** A stray `.json` in `examples/` changes `collected=` and can turn the gate red, which — with the commit hook — blocks every commit in the repository. Check `git status --untracked-files=all` before believing a census disagreement is real.
