...
**R4g — PRUNE-AND-RE-ENVELOPE REBUILD, BUILT, GATED GREEN, AMBER, STOPPED
FOR PATRICK'S CHECK — [`0180-report.md`](handoff/0180-report.md),
answering [`0179-ruling.md`](handoff/0179-ruling.md), on branch
`roofs-r4g-full-footprint` (off `roofs-r4f-multi-ridge`, PR #60,
unmerged; PR #59 closed in favour of it).** 0179 endorsed 0178's own
anchor/candidacy work ([`0178-report.md`](handoff/0178-report.md)) but
ruled its rank-walking orphan fallback wrong; replaced with a genuine
fixed point (a pruned roof's ground re-envelopes from scratch among
whoever remains, never a fixed rank order computed once). **A second,
independent bug found closing it**: restricting a roof's reach to its
own territory only (0178's own interim shape) broke the ordinary two-
roof "joining end extends, up to the seam" mechanic every T/L pair
depends on (measured: the equal-height-L poke-through fix regressed).
Fixed by matching `clip_pair`'s own untouched `ra_idx | ov_idx` allowed
set, generalised to N roofs: a roof's reach may walk its own territory
PLUS any contested (multi-coverer) piece from ANY current owner, never
another roof's own exclusive root ground. Gap ratio on the fixture:
~27% (0178) → ~15% now. Gate GREEN, 1275 passed. **Still not the
ordered 0% blank** — one diagnosed cause, not several (see
`0180-report.md` sec4 before touching this area again): a root
component certifies only the BIGGEST connected group of a roof's own
single-coverage ground as its anchor; a roof whose real ground
genuinely splits into two or more separate components (rf3, on this
fixture, and in miniature the equal-height-L's own outer corner) only
gets one certified. Two fixes were tried (validate every root
component; validate every component above a size threshold) and BOTH
reopen D85 outright, measured directly — a criterion that tells a real
second wing apart from D85's own excluded, seam-losing corner is named
as the next step, not guessed at under this ruling either.
