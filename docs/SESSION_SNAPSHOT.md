...
**R4g — CANDIDACY/REACHABILITY REBUILD, BUILT, GATED GREEN, AMBER, STOPPED
FOR PATRICK'S CHECK — [`0178-report.md`](handoff/0178-report.md), on
branch `roofs-r4g-full-footprint` (off `roofs-r4f-multi-ridge`, PR #59
still open/AMBER, unmerged).** Answers
[`0176-ruling.md`](handoff/0176-ruling.md)/[`0177-ruling.md`](handoff/0177-ruling.md):
candidacy in `compute_roof_clips` widens to every other live roof
(`_strip` itself untouched, still `clip_pair`'s shape); reachability
anchors at the connected component holding each roof's own single-
coverage ground, marker_end fallback otherwise. A real, pre-existing bug
found along the way (not merely the ordered candidacy/anchor change): the
orphan-reassignment fallback defaulted unconditionally to the WORST-
ranked coverer at 3+ candidates, which was the actual mechanism behind
most of the fixture's 27/44-boundary residual — fixed to leave a piece
undrawn rather than manufacture a wrong owner. `fp3d.py`'s gable-end
triangle also fixed (0174 sec5's own named method). Gate GREEN, 1275
passed (was 1257). **Two of the ruling's own receipts are honestly NOT
fully met** — read `0178-report.md` sec5 before touching this area
again: "exactly two valleys at the pinch" (only one draws) and the named
rf3-rake-over-rf1-eave jump (the underlying fact is verified, but neither
roof currently draws that ground) are both real, measured residuals, not
oversights — three different mechanisms were tried and reverted because
each one reopened D85 or the equal-height-L poke-through fix or the
interior-jump invariant instead.
