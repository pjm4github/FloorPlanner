...
measurement with one overclaim corrected and one demand — BUILT ON THIS
BRANCH (`roofs-r5e-grip-clamp`), AMBER, STOPPED FOR HIS CHECK:** the
dormer's back-end drag is clamped to the RISING side of the host's
surface inside its footprint (`roofclip.rising_reach`; past the crest
the grip stops dead instead of folding back), two fail-first tests plus
the pinned rising side; the closure below the dormer's eaves is NAMED
(R4f's `_cross_roof_risers` along the unequal-height boundaries) and
PROBED (rays through both eaves planes and the front hit; with the
risers removed they miss); 0194 §1's overclaim corrected and PR #64's
merge recorded in [`0196-report.md`](handoff/0196-report.md), which
lands on `main`, not here. Next: R6 multifloor, waiting on his fixture
and his one-line display-rule answer ([`0186`](handoff/0186-ruling.md)
§4).**
