...
**Re-cut 2026‑08‑08 on `a1-d47-fragment-extract`, with `main` @ `a1172be`; amended the same day when A1's manual check came back.** **PHASE 4 IS COMPLETE; the GREEN batch has merged; A1 PASSED ITS MANUAL CHECK and is merging, and that check inserted a new item ahead of A2.** This file exists so a fresh session can start from disk instead of from a chat summary. It is an **index and a state marker, not a second copy of the record** — where it points at another document, that document is authoritative and this one must not be trusted over it.
...
| | |
|---|---|
| **Branch** | `a1-d47-fragment-extract`, pushed. **PR #17's six-item manual check PASSED** (recorded verbatim on the PR) and it merges on that ruling. |
| **`main`** | `a1172be` — PR #16 (viewer floors measurement, plan-snap, and the queued batch). `origin/main` is the same commit. |
| **Merged since the last re-cut** | the whole **GREEN batch** — G1/D43 (PR #12), G3/D27 (PR #13), G2/D48 (PR #14), G4/D42 (PR #15) — then PR #16. |
| **PR #17 — A1 (D47)** | **PASSED and merging.** CI green on all six jobs (ruff · records · py3.10 · py3.13 · **windows** · deep invariants). The check confirmed three floating pieces, no group boxes, each carrying its region and label, no dashed edge, undo restoring both rooms — **and volunteered a measurement nobody asked for: totals 505 → 424 sq ft, held at 424 through the drags.** That 81 sq ft is the overlap that stopped being double-counted, which is independent evidence the pieces are disjoint; the scripted items could all have passed with two pieces still sharing area. **It also produced D53 and D54**, neither of which any automated signal had. |
| **Census** (full mode, this branch, 2026‑08‑08) | `collected=639 ruff=clean vacuous=0 end_assign=0`; OFF / ON / DEEP each **632 passed, 7 deselected**, every sum reconciling; **`Gate-Verdict: GREEN`**. **Zero xfails** — A1 flipped the suite's last one. |
| **Records** | **56 records — 16 open, 40 closed.** `python tools/gate.py --docs` **GREEN** (front matter valid, `INDEX.md` matches a regeneration, every defect reference resolves, migration dry run clean). `Ref-Audit: unresolved=10` — dangling *links and paths*, which are reported and never enforced; dangling *keys* would fail. |
| **Working tree** | clean, including untracked. **`fixtures/` is new** — manual-check plans, deliberately outside `examples/` so the corpus glob never sees them (see §6 and D51). |
...
1. **A1 — D47, `fragment` produces floating rooms via `extract`. AT ITS PULL REQUEST (#17), NOT MERGED.** `room_boolean("fragment")` now calls `extract_room` per region instead of wrapping that region's walls in a `GroupItem`. Differential on the record's two-room case: `room_owns_walls` false for all nine (group, room) pairs → **true for all three rooms**; drag +300/+300 moved 4 of 4 walls and **0 of 16 outline corners** → **4 of 4 corners and 4 of 4 walls**; `open_edges` `{2,1,1}` each with a real wall on it → **`{0,0,0}`**; groups created 3 → **0**; orphan walls 4 → **0**; `check(deep)` CLEAN throughout. **Nothing else starts until Patrick reports the manual check.**
2. **A1b — D53, a room cannot be selected by clicking its region. INSERTED 2026‑08‑08 AHEAD OF A2, by ruling.** Found at A1's manual check, and it is a **missing capability, not a regression** — the differential shows the room region selected the room in **zero of eight cases on BOTH trees**, so there is no commit that broke it; A1 merely removed the `GroupItem` that was masking its absence, which is what its own item 2 required. Cause: `RoomItem.shape()` returns only the label rect (`rooms.py:775`), and `PlanView` treats a press with no item under it as empty canvas — it pans **and calls `clearSelection()`** (`view.py:359‑365`), so pressing a room's region actively *de*-selects. Scope ruled in full: **(a)** a region click selects the room and persists after release **with a visible selected state**; **(b)** shift-click toggles membership, ctrl-click likewise; **(c)** rubber-band unchanged. Tier AMBER. **Do not fold the fix into A1's branch.** **Three constraints ruled 2026‑08‑08 before any code**, and the census they asked for is done (`docs/evidence/d53-hit-census.txt`): walls (`WALL_Z` 5.0) and openings (6.0) sit **above** the room and there is **no vertex item at all** — corner grabs live inside `WallItem` — so that half is already safe; but **furnishings (z 3) and groups (z 1) sit BELOW**, so a naive `shape()` widening makes every furnishing in the plan unclickable. The *empty canvas* idiom has **four** consumers, not one, and **middle-mouse pan already exists unconditionally** (`view.py:310‑314`). Rubber-band is independent of `shape()` in both halves — by two type filters, asserted nowhere. **The mutation is part of the receipt:** severing `_update_edit_actions` from `_sel_order` must BREAK the suite. **A1b's read-back goes to Patrick BEFORE implementation.**
3. **A2 — D11's RUNTIME z-order collapse. Now BEHIND A1b.** It hangs `test_drag_split_macro_keeps_every_room_rectilinear` at the first drag, bisected to `geometry.py`, and the trigger is the **magnitude** of the z step (`×1.0` completes, `×Z_STACK_BAND` hangs); the work was reverted, so nothing of it is in the tree and it is **not reproducible from disk**. **The roadmap's instruction is explicit and is the first step: INSTRUMENT THE HANG WITH A BOUNDED EVENT COUNTER TO FIND THE CONSUMER OF THE z STEP. Do not choose constants that make the symptom go away.** The agreed rule (ruling 4) carries forward whole: z = `floor_term + stack_term + type_term`; the backdrop's −1e9 becomes a **type term**, not a magic number; `bring_to_front`'s full-scene max scan dies with it; the band arithmetic becomes **named constants** with `max(type_term) < STACK_BAND` and `max(stack_term) < FLOOR_BAND` written beside them and **pinned by a test**. Manual check: bring-to-front sticks; ghost floors stay behind the active floor; multi-floor banding unchanged.
4. **A3 — D11's SERIALIZATION half.** No longer blocked: **ruling R‑B** (roadmap §2) decided that an *additive optional* field or enum value does not bump the document version, so a stacking index can be added at `schema_revision` without a v6. Still AMBER.
5. **A4 — D49, the deep checks at document boundaries. AMENDED 2026‑08‑07 — read the amendment, not the 2026‑08‑05 proposal it supersedes.** I11 speaks nowhere in the shipped app: deep-only *plus* shadow mode off by default compose into a hole neither has alone. Measured on one corrupt scene: `FP_VERIFY_DESIGN` unset → **the save wrote the file** with I5b ×1 and I11 ×3; `=1` → refused; `=deep` → refused. The ruling: **CHECK YES, FIX NO** (overlap's causes vary and every auto-repair silently changes drawn geometry; the one repairable cause is already fixed by welding on load — same reasoning as **D34**); **SAVE ASKS, IT DOES NOT REFUSE** (a deform-to-follow drag can transiently overlap, and a hard refusal traps the user with unsaveable work); and **the report must be ACTIONABLE** — rooms *and overlap area*, plus select-and-zoom. **Sequencing, measured:** the area does not exist in I11 today (a boolean from three terms, no number), computing it honestly **is D52's half 1**, and on the amendment's own driving case the true intersection is **0.0 sf** — so `planc1` is the acceptance case and `farmplace` is the silence case. Real behaviour change; will be felt.
6. **A5 — D41, the new simple-ring invariant.** Ruled at **R‑A**: a ring that visits a vertex twice is a *degeneracy*, not a crossing, so it gets its own invariant rather than widening I5b. **A read-back is required before starting** — which files in `examples/` fail it, deep-only or one of the cheap twelve, and the corpus-freeze diff for `symmetricP1.json`.
7. **A6 — Grid snap.** Three sub-rulings still RED (see §4 of the roadmap).
8. **Phase 5 — Landscape** (P5.1 site levels/categories/area accounting, P5.2 landscape wall types + gates, P5.3 site schedule fields + reports).
