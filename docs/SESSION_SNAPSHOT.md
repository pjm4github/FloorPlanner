...
landed on `main` by this merge commit on Patrick's word ("that fixed it,
merge PR #62"), the branch deleted in the merge step; reports
[`0188`](handoff/0188-report.md) (built: every wall piece capped at the
surface of the roof whose territory it lies under, `_wall_under_roofs`)
and [`0189`](handoff/0189-report.md) (his first look: the cap moved to
the roof's underside plus ½″, `WALL_CAP_BELOW_ROOF_IN`, so no wall face
coincides with a roof face). THE R5b DORMER READ-BACK IS ON DISK —
[`0190-report.md`](handoff/0190-report.md), this commit — RED UNTIL HE
RULES: a dormer is a `roof` record with one additive `host` field, its
back ridge end derived into the host plane (measured: today's clip
already produces the right dormer with no special case once the back
end is at or past the meet; the one 2D change is `roof_clip_spans`
reading territories — the host falsely dashes a wall under a dormer);
cheeks and face derived, drawn dashed in plan and built in 3D as prisms
between the host plane and the dormer's own; the gesture drops on a
roof plane snapped to the R5a trace, drag along it for width, ridge
up-slope by default; v1 gable, hip nearly free, shed its own ruling.
NOTHING ELSE IS OWED; no branch is open.** `viewer/fp3d.py`: the roof clips are computed
