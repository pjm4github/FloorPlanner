...
| | |
|---|---|
| **`main`** | **`a1fe005`** at this file's cut. **`0098`-`0103`'s wall id/coordinate work is DONE on both PR branches**: the map (`design_from_scene`'s `report["wall_items"]`), both ids + endpoints in feet on every row, and the shared `WallRowList` widget (click centres + selects; a dead row is one whose id fails a fresh round-trip walk, catching a MERGED wall same as a removed one) — wired into `OrthogonalityReportDialog` (PR #39, `wall-report-id-fix`) and `OrthogonalityRepairDialog`'s preview (PR #37, `wall-orthogonality-repair`, duplicated per-branch). **PR #37's check is UNBLOCKED and ready** — `fixtures/wiscaway2026-08-09R.json`, expect 5 walls, largest correction 0.041″. The mailbox hook's duplicate-handoff-number check is DONE (`.claude/hooks/verify_gate.py`, `tests/test_verify_gate_hook.py`). **`0104-ruling.md` landed**: refuses change-impact test selection (star imports + late-import cycle + mutable `SETTINGS` global make "what a feature touches" uncomputable); confirms the quick/full split is already in use; orders per-task progress files as the blocker on parallel agents — **DONE this cut**: `docs/progress/tasks/`, generated by `tools/progress_index.py` (`Docs-Progress` in `gate.py --docs`), historical phase files left frozen/unmigrated on purpose. **Owed next: `0104` §5 tier 2, the ON/DEEP-vs-OFF divergence measurement**, before any ruling on cutting the 3×. PR #38 (the angle clause) unaffected, still worth checking. **Numbering collisions, on the record:** `0036`, `0043`, `0050`, `0101`; neither renamed, numbering continues forward each time. Full trail: `handoff/README.md`'s pair table. |
