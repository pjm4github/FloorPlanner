...
| | |
|---|---|
| **`main`** | **`8eb428f`** at this file's cut. PR #39/#37/#38/#40/#41/#42/#43/#44/#45/#46/#47 all merged. **`0108`-`0110`'s three snap-to-grid features COMPLETE. ITEM C CLOSED** (`0066`/`0137`/`0138-report.md` — zero walls flagged). **THE ROOFLINE PLAN — [`0139-ruling.md`](handoff/0139-ruling.md) (authored `0138`, renumbered on landing), BUILDING NOW.** Five gated tranches, one branch/PR/check each, next starting only after the previous merges: **R1** roofs in the v5 document (model + round-trip + v5→6 migration, NO UI) — GREEN, merge on green CI, no check owed; **R2** roof menu + ridge sketch + heights dialog — AMBER; **R3** roof planes + gables in `fp3d.build_model` — AMBER; **R4** the parameters dialog — AMBER; **R5** dormers — RED, own ruling later. Named not ordered: valley/hip lines, a roof-plan export sheet, yard items (deferred, not lost). Carried, undated: `0134`'s PDF-tranche one-word check; room-label rounding ([`0131`](handoff/0131-ruling.md) §2); delta-snap sites; D61-family. Full trail: `handoff/README.md`'s pair table. **Numbering collisions, on the record:** `0036`, `0043`, `0050`, `0101`, `0138`; neither renamed, numbering continues forward each time. |
