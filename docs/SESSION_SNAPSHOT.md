...
| | |
|---|---|
| **`main`** | **`ef38ef3`** — this copy is carried onto `wall-orthogonality-repair` while bringing that branch current with `main` (actually at `ecf91bd`, all of `0098`-`0107` plus the `0112` cleanup). **`0108`-`0110` spec three new AMBER features** (all queued behind PR #37/#38, per `0111`): "Snap to Grid" (per-wall, manual, AMBER), "Snap to Grid Orthogonal" (the safer variant, build **first**), a disabled placeholder menu item for the 15°-angle case (GREEN, no arithmetic exists for it — `tan 15°` is irrational, proven in `0110` §3). **`0111`/`0112` order the close-out, IN PROGRESS**: `wall-report-id-fix` current with `main`, pushed; **now bringing `main` into `wall-orthogonality-repair`, this merge** — next `wall-label-angle-clause`, then one check session in order: `wall-report-id-fix` → PR #37 → PR #38 (three branches need a check, not two). Nothing else opens a branch until those three merge — `0072` §7's three-concurrent-AMBER-PRs bottleneck has rebuilt itself. **Numbering collisions, on the record:** `0036`, `0043`, `0050`, `0101`; neither renamed, numbering continues forward each time. |
