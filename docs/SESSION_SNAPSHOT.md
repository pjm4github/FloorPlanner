...
| | |
|---|---|
| **`main`** | **`7215b23`** at this file's cut. PR #39/#37/#38/#40/#41/#42/#43/#44/#45/#46 all merged. **`0108`-`0110`'s three snap-to-grid features COMPLETE.** **Still open, Patrick's alone, no code blocks it: the `L2.dxf` Chief recount** (`0066` §6/§7, **still the oldest open line on the board** — the repair tool is already live on `main`, purely his own manual step, no code). **PDF cleanup tranche (D82/family exclusivity) MERGED — [PR #46](https://github.com/pjm4github/FloorPlanner/pull/46) → `main` at `bb9309a`** — full trail [`handoff/0134-report.md`](handoff/0134-report.md). **`fp2dxf`'s door-symbol-by-type + real windows — BUILT, GATE GREEN, PR PENDING on `fp2dxf-door-symbols`**, per [`0135-ruling.md`](handoff/0135-ruling.md): the `door_type == "sliding"` dead check (D81's sibling defect in the DXF exporter) fixed with one symbol per catalog type; windows gain a real glass line; the golden fixture `sample_design.json` itself was found carrying the exact bogus values (`"sliding"`/`"hinged"`) and corrected in the same commit. Full trail: [`handoff/0136-report.md`](handoff/0136-report.md), `handoff/README.md`'s pair table. **Numbering collisions, on the record:** `0036`, `0043`, `0050`, `0101`; neither renamed, numbering continues forward each time. |
| **Branches** | **`fp2dxf-door-symbols`** — AMBER, PR pending, `0135-ruling.md`'s door-symbol-by-type + real windows, stopped for Patrick's check in Chief (re-export/re-import R2). **`Edit ▸ "Repair wall orthogonality…"` (item C) is already live on `main`** (`mainwindow.py:368`) — nothing to build or merge for it; `0066-ruling.md` §7 leaves only **Patrick's own manual step**: open the plan that produced `L2.dxf`, run the repair, export via File ▸ Export ▸ Chief Architect (DXF)…, open in Chief, count flagged walls against the 75 baseline, confirm the drawing still looks right. No branch, no PR, no code. PR #37/#38/#39 confirmed merged 2026‑08‑26 (a stale "AMBER, open" claim here corrected 2026‑08‑31). `dimension-cleanup-tranche`/`angled-dimension-lanes`/`dim-row-along-refactor`/`pdf-dimension-and-door-fix`/`export-menu-pdf` merged and deleted, local and remote. `fp2dxf-integration` deleted earlier, joining the five from [`0053`](handoff/0053-ruling.md) §2 item 4. |
