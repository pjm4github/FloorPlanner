...
| | |
|---|---|
| **`main`** | **`350e840`** at this file's cut. PR #39/#37/#38/#40/#41/#42/#43/#44/#45/#46/#47 all merged. **`0108`-`0110`'s three snap-to-grid features COMPLETE.** **ITEM C IS CLOSED** — [`0066-ruling.md`](handoff/0066-ruling.md)'s wall orthogonality repair, the board's oldest open line (since 2026‑08‑23): Patrick's own two-part check both passed — the drawing "looks fine" ([`0137`](handoff/0137-ruling.md)) and **zero walls flagged** by Chief on `L1`/`L2`, against the 75 before ([`0138-report.md`](handoff/0138-report.md)). No code changed; the repair has been live on `main` since PR #37 (merged 2026‑08‑26) — this closed the verification, not the build. **PDF cleanup tranche MERGED** ([PR #46](https://github.com/pjm4github/FloorPlanner/pull/46), `0134-report.md`). **`fp2dxf`'s door-symbol-by-type + real windows MERGED** ([PR #47](https://github.com/pjm4github/FloorPlanner/pull/47), `0136-report.md`) — Patrick's own words: "The L1 and L2 dxf layers work fine." **No AMBER/RED item currently open.** Carried, undated, no deadline: `0134`'s PDF-tranche one-word check; room-label rounding ([`0131`](handoff/0131-ruling.md) §2); delta-snap sites; D61-family. Full trail: `handoff/README.md`'s pair table. **Numbering collisions, on the record:** `0036`, `0043`, `0050`, `0101`; neither renamed, numbering continues forward each time. |
| **Branches** | **No branch open on this project's own work.** `Edit ▸ "Repair wall orthogonality…"` (item C) is already live on `main` (`mainwindow.py:368`) — nothing to build or merge for it; `0066-ruling.md` §7 leaves only **Patrick's own manual step, the last thing open**: open the plan that produced `L2.dxf`, run the repair, export via File ▸ Export ▸ Chief Architect (DXF)…, open in Chief, count flagged walls against the 75 baseline, confirm the drawing still looks right. No branch, no PR, no code. `fp2dxf-door-symbols`/`dimension-cleanup-tranche`/`angled-dimension-lanes`/`dim-row-along-refactor`/`pdf-dimension-and-door-fix`/`export-menu-pdf` merged and deleted, local and remote. `fp2dxf-integration` deleted earlier, joining the five from [`0053`](handoff/0053-ruling.md) §2 item 4. |
