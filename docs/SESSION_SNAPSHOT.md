...
| | |
|---|---|
| **`main`** | **`09fc575`** at this file's cut — landed item C's displacement instrument + read-back ([`0066-ruling.md`](handoff/0066-ruling.md), [`0079-report.md`](handoff/0079-report.md)): the repair's tolerance is a DISPLACEMENT (inches), not a degree; the conflict predicate implemented and measured (14 of 63 near-axis walls conflict, only 2 truly unrepairable, 61 of 63 repaired). **The repair itself is not built** — AMBER, blocked on the read-back being ruled. **This commit lands `0077`/`0078` too** — [`0080-report.md`](handoff/0080-report.md): a real bool-parsing gap in `coerce_setting` fixed (`bool("false")` was `True`), `reportlab` into `requirements-dev.txt` (its receipts now actually render a PDF in CI), `_stdt.py` decouples `fp2pdf` from `fp2dxf` entirely, a corrupt settings file quarantined instead of silently emptied, full materialisation plus `SETTINGS_VERSION`/a migration table/a mechanical pin test. **Still AMBER/RED and not built**: the three-rung precedence chain, the checkbox, the export menu, the PDF dialog, `--settings`, the orthogonality repair. Full trail: `handoff/README.md`'s pair table. |
