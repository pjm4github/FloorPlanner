...
| | |
|---|---|
| **`main`** | **`0224a52`** at this file's cut. PR #39/#37/#38/#40/#41/#42/#43/#44 all merged. **`0108`-`0110`'s three snap-to-grid features COMPLETE.** **Still open, Patrick's alone, no code blocks it: the `L2.dxf` Chief recount** (`0066` §6's closing receipt for item C). **D81/D82 CLOSED** — full trail: [`handoff/0121-report.md`](handoff/0121-report.md), [`0122-report.md`](handoff/0122-report.md). **TRANCHE 2 (aligned dimensions for angled walls), per [`0123-ruling.md`](handoff/0123-ruling.md):** **1. `dim_row_along` refactor MERGED** — [PR #44](https://github.com/pjm4github/FloorPlanner/pull/44) → `main`, GREEN, no check owed. **2. The room-`show_dimensions`-driven angled lane — CHECK FEEDBACK RECEIVED AND FIXED, PR STILL OPEN on `angled-dimension-lanes`.** Patrick's first check found three faults (door positions listed, lanes not snug, callouts crossing the drawing) — [`0127-ruling.md`](handoff/0127-ruling.md), all adopted — plus a follow-up dropping opening stations from ALL rows, not just the angled lanes — [`0128-ruling.md`](handoff/0128-ruling.md). Both fixed on the same branch, per [`0126-report.md`](handoff/0126-report.md): lane baseline now anchors to the family's own outer envelope (not the whole plan bbox), `_features()` drops opening centrelines, a footer note added, `wiscaway2026-08-30R1.json`/`DIM45.png` promoted out of `fixtures/incoming/`. **Re-check pending, same PR.** Full trail: `handoff/README.md`'s pair table. **Numbering collisions, on the record:** `0036`, `0043`, `0050`, `0101`; neither renamed, numbering continues forward each time. |
| **Branches** | **`angled-dimension-lanes`** — AMBER, PR pending, `0120-ruling.md`'s room-driven angled dimension lane, stopped for Patrick's check (Wiscaway, 45° rooms with Show dimensions on). **`wall-orthogonality-repair`** ([PR #37](https://github.com/pjm4github/FloorPlanner/pull/37), item C) — AMBER, open, **check UNBLOCKED, ready to run** (against `wiscaway2026-08-09R.json`). **`wall-report-id-fix`** ([PR #39](https://github.com/pjm4github/FloorPlanner/pull/39), the standalone report's own fix) — AMBER, open, ready. **`wall-label-angle-clause`** ([PR #38](https://github.com/pjm4github/FloorPlanner/pull/38), the angle clause) — AMBER, open, worth checking, unaffected. `dim-row-along-refactor`/`pdf-dimension-and-door-fix`/`export-menu-pdf` merged, not yet deleted (remote branches kept per `--delete-branch=false`). `fp2dxf-integration` deleted, local and remote, joining the five from [`0053`](handoff/0053-ruling.md) §2 item 4. |
