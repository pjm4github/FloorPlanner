...
| | |
|---|---|
| **`main`** | **`daf1449`** at this file's cut — landed every GREEN item across `0072`-`0075` (`0076-report.md`: `coerce_setting()`, the JSON settings store, migration, `fp2pdf.py`'s four hygiene fixes). **This commit lands item C at last** — [`0066-ruling.md`](handoff/0066-ruling.md) rules the repair's tolerance is a DISPLACEMENT (inches), not a degree; [`0079-report.md`](handoff/0079-report.md) builds the displacement instrument (cross-checked exactly against the ruling's own 63-value list) and answers the read-back item 2 (the repair itself) is blocked on — the conflict predicate implemented and measured (14 of 63 near-axis walls have a conflict, only 2 truly unrepairable). **The repair itself is not built** — AMBER, blocked on this read-back being ruled. Also owed: `0077`/`0078`'s further settings/`fp2pdf` GREEN items (a real bool-parsing gap in `coerce_setting`, `_stdt.py` decoupling, materialisation + a version/migration guard). Full trail: `handoff/README.md`'s pair table. |
