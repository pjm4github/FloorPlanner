...
make sure that the script tool has commands to handle tha dormer"* —
BUILT, AMBER, STOPPED FOR HIS CHECK: [PR #64](https://github.com/pjm4github/FloorPlanner/pull/64)
on `roofs-r5c-undo-macro`, report [`0193-report.md`](handoff/0193-report.md)
(this commit lands it).** (1) The undo was already there — undo is
snapshot-based off the scene's own change signal, so deleting a host
with its dormers was always one undo step; **0192 §3 said otherwise and
0193 §1 corrects it**, two tests pin it. (2) The macro gains a
self-contained `DORMER x y width eaves ridge [dx dy]` token on the
DOOR/WINDOW/ROOM pattern — the recorder emits it when a dormer is
finished through the tool (dialog values baked in, the raw drag
suppressed), the runner builds the dormer with no dialog;
`docs/macro_language.md` updated. **AND HIS LOOK AT THE MERGED DORMER,
fixed on the same branch — [`0194-report.md`](handoff/0194-report.md),
which this commit lands:** *"There is a missing control knob on the
ridge of the dormer where it meets the roof. In addition the walls of
the dormer are being drawn down from the dormer. It should show only
the roof line and intersect with the walls under it."* — the back-end
grip is back and sets the RIDGE HEIGHT from the host plane at the
dragged point; the dormer's own cheeks and face are gone from plan and
3D, and the house walls under a dormer's territory climb into its roof
(`_wall_under_roofs`, piers and headers only). His check, now three
things: record a dormer and replay it; delete a host and undo it; drag
the dormer's back knob and see the walls under it climb into its roof
in 3D.
