...
trace, `fixtures/dormer-gable-check.json` promoted. **HIS NEXT
INSTRUCTION, given with that check and NOT YET BUILT:** *"lets do the
undo. also make sure that the script tool has commands to handle tha
dormer"* — (1) roof deletion undoable (0192 §3's named limit: no roof
deletion was ever undoable; deleting a host takes its dormers), (2) the
macro language records and replays the Dormer tool end to end, like the
ridge tool. His own direct instruction as authority (`0065` §2, quoted
in the report that builds it); AMBER, its own branch and PR, stopped for
his check. Then R6 multifloor, on his fixture and his display-rule answer
