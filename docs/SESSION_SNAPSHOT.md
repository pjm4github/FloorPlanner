...
| | |
|---|---|
| **`main`** | **`cc12bbf`** at this file's cut — `444996a`/`404ed89`/`f93e9dd` (the orthogonality band-split correction, the `0061`-`0063` mailbox, `incoming/`'s fourth exit — see `handoff/README.md`'s pair table for the full trail), then two outside the handoff queue, Patrick asking directly in-session: `5d85b09` put the selected wall's id + its two vertex ids on the status bar, `cc12bbf` extended that label with each vertex's (x, y) in decimal feet (3 sig figs), the wall's length the same way, and its heading in degrees when not exactly axis-aligned (`WallItem.uid` mirrors `Vertex.uid`; `geometry.py` gained `fmt_ft3`/`heading_deg`). |
