...
| | |
|---|---|
| **`main`** | **`31300e8`** at this file's cut — `fe8f5b1` re-cut the marker, then `31300e8` landed `floorplanner/export/fp2pdf.py` (Patrick's new PDF exporter, ruff-only fixes, not yet wired in). This commit lands `0070`-`0073`: `0070`/`0071` are Patrick's own off-grid report, reproduced, root-caused and fixed — [D80](defects/0080-a-fresh-wall-started-at-a-t-junction-inherits.md); `0072`/`0073` are Patrick's `fp2pdf` wiring request and its settings-precedence amendment, both ruled, mostly AMBER/RED, one GREEN step each (hygiene fixes; a type-aware settings loader) not yet built. Full trail: `handoff/README.md`'s pair table. |
