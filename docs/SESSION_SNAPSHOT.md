...
| | |
|---|---|
| **`main`** | **`55983a4`** at this file's cut. PR #39/#37/#38/#40/#41/#42/#43/#44/#45/#46/#47 all merged. **`0108`-`0110`'s three snap-to-grid features COMPLETE. ITEM C CLOSED** (`0066`/`0137`/`0138-report.md` — zero walls flagged). **THE ROOFLINE PLAN — [`0139-ruling.md`](handoff/0139-ruling.md) (authored `0138`, renumbered on landing), BUILDING.** [`0140-ruling.md`](handoff/0140-ruling.md) sharpens R2b (the End-On marker/dialog, Patrick's sketch) — no new tranche. [`0141-report.md`](handoff/0141-report.md): a second numbering collision recovered (Cowork's End-On ruling landed as `0139` too, overwriting Code's already-committed file on disk; recovered to `0140`), and **R1 built** (model + round-trip + v5→6 schema migration, no UI) — gated GREEN, not yet branched/PR'd. Five gated tranches total, one branch/PR/check each, next starting only after the previous merges: **R1** roofs in the v5 document, NO UI — GREEN, merge on green CI, no check owed, **next up: branch + PR**; **R2** roof menu + ridge sketch, heights dialog now minimal (initial heights only) — AMBER; **R2b** the End-On marker + end-on view + three-way ridge/eaves/pitch dialog ([`0140`](handoff/0140-ruling.md)) — AMBER; **R3** roof planes + gables in `fp3d.build_model` — AMBER; **R4** the parameters dialog — AMBER; **R5** dormers — RED, own ruling later. Named not ordered: valley/hip lines, a roof-plan export sheet, yard items (deferred, not lost). Carried, undated: room-label rounding ([`0131`](handoff/0131-ruling.md) §2); delta-snap sites; D61-family. Full trail: `handoff/README.md`'s pair table. **Numbering collisions, on the record:** `0036`, `0043`, `0050`, `0101`, `0138`, `0139`; neither renamed after commit, numbering continues forward each time. |
| **Branches** | **No branch open on this project's own work** — R1's code (model.py/schema/tests) sits stashed in the working tree, about to move to its own branch. `Edit ▸ "Repair wall orthogonality…"` (item C) is already live on `main` (`mainwindow.py:368`) — nothing to build or merge for it. `fp2dxf-door-symbols`/`dimension-cleanup-tranche`/`angled-dimension-lanes`/`dim-row-along-refactor`/`pdf-dimension-and-door-fix`/`export-menu-pdf` merged and deleted, local and remote. `fp2dxf-integration` deleted earlier, joining the five from [`0053`](handoff/0053-ruling.md) §2 item 4. |
