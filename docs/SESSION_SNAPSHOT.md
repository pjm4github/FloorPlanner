...
[`0190`](handoff/0190-report.md)'s read-back — BUILT ON THIS BRANCH
(`roofs-r5b-dormers`), AMBER, STOPPED FOR PATRICK'S CHECK.** A dormer
is a `roof` record with an optional `host` (schema, `Roof`, writer,
loader with a dangling-host skip, `canonicalize` remapping the
reference); its back end is derived where the ridge meets the host
plane (`roofclip.meet_along`) and still written; cheeks and face are
derived plan lines and, in `fp3d`, prisms standing on the host plane;
`roof_clip_spans` reads a roof's drawn territory; the Dormer tool is its
own Roof-menu item, toolbar button and macro letter `M` (press on a roof
plane snapped to the R5a trace, drag along it for width, Shift frees
the direction, the End-On dialog refuses a ridge the host never meets);
`fixtures/dormer-gable-check.json` promoted. Report
[`0192-report.md`](handoff/0192-report.md) lands on `main`, not here.
His check (0191 §3): sketch a dormer on the loft from the Dormer tool —
the trace and the room's dashes open under it, the valleys draw solid,
the back end lands on the host plane by itself — then the same in 3D.
**THE 3D WALL CLIP IS CLOSED AND MERGED** ([PR #62](https://github.com/pjm4github/FloorPlanner/pull/62),
[`0188`](handoff/0188-report.md)/[`0189`](handoff/0189-report.md)); R6
waits on his fixture and display answer ([`0186`](handoff/0186-ruling.md) §4).** `viewer/fp3d.py`: the roof clips are computed
