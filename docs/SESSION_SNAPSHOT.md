...
| | |
|---|---|
| **`main`** | **`35dedf6`** at this file's cut. **[`0095-report.md`](handoff/0095-report.md) is the current status** — read that first: `0084-ruling.md` §6 items 1-3 are BUILT on `wall-orthogonality-repair` (not yet committed there as this row is cut) — `T` restored as the candidacy filter, the orthogonality post-condition added (and a real exemption bug in its own first draft found and fixed against `wiscaway`'s `w57`), the corpus census re-run: the whole-file rollback no longer fires at all (0 of 63, was 37 stranded) — item 3's "AMBER to change" half turned out unneeded. **PR #37 becomes worth checking next.** Also landed: `0068` §4 (heading vs. deviation) is ANSWERED — `0092`/`0093`/`0094` rule "both numbers", suppressed on-intended-angle (the 15° `rotate_snap_deg` grid, Ctrl-drag's own snap), a real self-found bug (exact-equality breaks on 15/30/60/75°, fixed with a 1e-9° tolerance) — AMBER, not yet built, batched with PR #37's check. `0085`'s `CLAUDE.md` trim is GREEN, unblocked, still owed. Full trail: `handoff/README.md`'s pair table. |
| **Branches** | **`wall-orthogonality-repair`** ([PR #37](https://github.com/pjm4github/FloorPlanner/pull/37), item C) — AMBER, `0084-ruling.md` §6 items 1-3 built, about to be committed and pushed. A second AMBER branch for `0092`-`0094` (the status-bar angle clause) is next, batched with #37's check. `fp2dxf-integration` deleted, local and remote, joining the five from [`0053`](handoff/0053-ruling.md) §2 item 4. |
