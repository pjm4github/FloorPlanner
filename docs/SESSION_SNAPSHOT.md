...
**R4g IS CLOSED AND MERGED — [PR #60](https://github.com/pjm4github/FloorPlanner/pull/60)
landed on `main` at `c52396b` on Patrick's word ("that looks absolutely
perfect!"), both roof branches deleted; closing report
[`0185-report.md`](handoff/0185-report.md). NOTHING IS OWED. The next
move is his: R5 dormers (RED behind [`0164`](handoff/0164-ruling.md) §3,
needs its own ruling), the named-not-ordered items (valley/hip plan
lines, a roof-plan export sheet, yard items), or the one open question
0185 §2 names (crossed arms that survive stay joined — rf2's 4" fascia
at the pinch is not drawn; one line if he wants it open).** The trail:
[`0181`](handoff/0181-ruling.md) → [`0182`](handoff/0182-report.md)
(built, 0% blank) → [`0183`](handoff/0183-report.md) (his check: 3D
"fantastic", a missing plan joint fixed — seams read off the final
pieces) → [`0184`](handoff/0184-report.md) (his short-ridge fixture: a
ridge end in the air over its host is an open gable end; a crossed end
never opens) → merged. Evidence:
`docs/evidence/threeridge-drawn-junction-0182.png` beside his v2
reference; fixtures `threeRidgeFloorplan.json` and
`threeRidgeShortRidgeFloorplan.json`, both his, both load-bearing.
...
count as pushes to the hook -- delete a merged ref through `gh api`.
