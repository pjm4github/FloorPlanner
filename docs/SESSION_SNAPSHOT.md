...
| | |
|---|---|
| **`main`** | **`ea189b2`** at this file's cut. PR #39, PR #37 (item C), PR #38 (angle clause), PR #40 (Snap to Grid Orthogonal) all merged and checked. **Plain "Snap to Grid" — BUILT, AMBER, branch `snap-to-grid`, stopped for Patrick's own check.** `floorplanner/design/validate.py`'s `snap_wall_to_grid` (both endpoints snap to grid INDEPENDENTLY, no anchor, no near-45 refusal — unlike Orthogonal, a wall can come out tilted if its ends straddle a grid line, a NAMED limitation not a bug) plus `WallItem._snap_to_grid` (always enabled, no endpoint hit test needed; MULTI-SELECT applies to every selected wall in turn, guards re-evaluated fresh before each one, 0108-ruling.md §4). Same refusal/report machinery as Orthogonal (degenerate refuse, new `check()` violation refuse via stable-key diff, worsened neighbour reported not refused). Verified against the real corpus: `wiscaway2026-08-09R.json`'s `w56` (the x≈78.97ft chain 0108's own check names) snaps both ends to exactly 79.00ft, reports `w55`/`w57` worsened, no new `check()` errors. Full trail: `handoff/README.md`'s pair table. **Numbering collisions, on the record:** `0036`, `0043`, `0050`, `0101`; neither renamed, numbering continues forward each time. |
