...
| | |
|---|---|
| **`main`** | **`4f753e2`** at this file's cut — PR #39 (`wall-report-id-fix`) JUST MERGED, Patrick's check passed (report rows name a findable wall, click centres + selects). **`0108`-`0110` spec three new AMBER features** (all queued behind PR #37/#38, per `0111`): "Snap to Grid" (per-wall, manual, AMBER), "Snap to Grid Orthogonal" (the safer variant, build **first**), a disabled placeholder menu item for the 15°-angle case (GREEN, no arithmetic exists for it — `tan 15°` is irrational, proven in `0110` §3). **`0111`/`0112`'s close-out, IN PROGRESS**: all three checks passed (PR #39, PR #37 — 5 walls/0.041″ largest as measured, PR #38 — angle clause). **Merging sequentially now, PR #39 first (this merge); PR #37 next, then PR #38**, each re-gating on the previous merge before it lands, each branch deleted (local + remote) after. **Numbering collisions, on the record:** `0036`, `0043`, `0050`, `0101`; neither renamed, numbering continues forward each time. |
