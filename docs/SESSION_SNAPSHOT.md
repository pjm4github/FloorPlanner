...
| | |
|---|---|
| **`main`** | **`63b6836`** at this file's cut. **`0098`-`0103`'s wall id/coordinate work is DONE on both PR branches** (`WallRowList`, click centres + selects, dead-row-via-round-trip) — PR #39 and PR #37, both AMBER, ready. **PR #37's check is ready** — `fixtures/wiscaway2026-08-09R.json`, expect 5 walls, largest correction 0.041″. **`0104` DONE**: refused change-impact test selection; `docs/progress/tasks/` built (`tools/progress_index.py`, `Docs-Progress` in the docs lane), historical phase files left frozen/unmigrated on purpose; the ON/DEEP-vs-OFF measurement (§5 tier 2) run across all 267 CI runs since the workflow began — **zero instances of ON/DEEP going RED while OFF was GREEN on a real fault**; every RED was snapshot staleness (37/47) or a docs cross-reference (7/47) or a base test failure (4/47, ON never ran). Full receipt: `0106-report.md`. **`0105` landed; the CI cuts are built and FIXED once already**: the first push (`63b6836`) broke CI outright — a job-level `if:` cannot read `matrix` context, confirmed empirically (the whole workflow failed to parse, zero jobs ran) — refixed with a per-step `if:` on the surviving job instead; CI's `pytest (py3.10)`, `pytest (windows)`, `pytest deep invariants` and `records (gate --docs)` jobs disabled (`if: false`, not deleted); the surviving `pytest (py3.13)` job now runs the full local gate; `requires-python` raised to `>=3.13`. **Gap flagged, not acted on**: `--docs`-specific checks are not part of `gate.py`'s bare/full mode, so cutting the CI docs job means nothing runs them anywhere now — a future ruling's call, per `0106-report.md` §3. PR #38 (the angle clause) unaffected, still worth checking. **Numbering collisions, on the record:** `0036`, `0043`, `0050`, `0101`; neither renamed, numbering continues forward each time. Full trail: `handoff/README.md`'s pair table. |
