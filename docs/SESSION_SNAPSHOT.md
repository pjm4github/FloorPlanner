...
both; D50 CLOSED (`closed_by b8475b7`). **HIS FIXTURE ARRIVED (0197 §3) — `fixtures/wiscaway-2level-stacked-floor.json`,
promoted under exit 1: L1 at 0″/96″ with two roofs, L2 at 100″/196″
with four — AND R6.a IS BUILT, AMBER, STOPPED FOR HIS CHECK:
[PR #67](https://github.com/pjm4github/FloorPlanner/pull/67) on
`roofs-r6a-display`, report [`0199-report.md`](handoff/0199-report.md)
(this commit lands it; 0199 opens with the record of PR #66's merge).**
The plan view draws this level's roofs solid and any other level's roof
that covers this level's rooms ghosted, even with "show other floors"
off (`roofs.roof_covers_floor`, R4b's room-coverage overlap reused;
measured on his fixture: every roof covers both levels' rooms but L1's
45° wing, the fail-first control); the 3D popup builds the whole
building (`show_3d_view` no longer narrows). His check: the fixture's
two plans with "show other floors" off, then the 3D view — both storeys
stacked, all six roofs, NOT yet composed across levels (R6.b). **On his
word: merge PR #67, delete the branch in the merge step.** Then R6.b
(composition over all live roofs in absolute height) → D67 reproduced
or refuted → R6.c (the roof tool across levels).**
