...
promoted from `incoming/` under exit 1: L1 at 0″/96″ with two roofs, L2
at 100″/196″ with four — AND R6.a IS BUILT ON THIS BRANCH
(`roofs-r6a-display`), AMBER, STOPPED FOR HIS CHECK:** the plan view
draws this level's roofs solid and any other level's roof that covers
this level's rooms ghosted, even with "show other floors" off
(`roofs.roof_covers_floor`, R4b's own room-coverage overlap reused;
measured on his fixture: every roof covers both levels' rooms but L1's
45° wing, which covers no upper room — the fail-first control); the 3D
popup builds the whole building (`show_3d_view` no longer narrows to
the active level; D68's boundary rule stands). Report
[`0199-report.md`](handoff/0199-report.md) lands on `main`, not here.
His check: open the fixture, switch levels with "show other floors"
off — the other level's covering roofs ghosted, the wing gone from the
upper plan — then the 3D view showing both storeys stacked. Then R6.b
