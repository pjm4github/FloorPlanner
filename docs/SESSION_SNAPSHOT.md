...
trace, `fixtures/dormer-gable-check.json` promoted. **HIS NEXT INSTRUCTION, given with that check — *"lets do the undo. also
make sure that the script tool has commands to handle tha dormer"* — BUILT
ON THIS BRANCH (`roofs-r5c-undo-macro`), AMBER, STOPPED FOR HIS CHECK:**
(1) the undo was already there — undo is snapshot-based off the scene's
own change signal, so deleting a host with its dormers was always one
undo step; 0192 §3 said otherwise and [`0193-report.md`](handoff/0193-report.md)
corrects it, two tests pin it; (2) the macro gains a self-contained
`DORMER x y width eaves ridge [dx dy]` token on the DOOR/WINDOW/ROOM
pattern — the recorder emits it when a dormer is finished through the
tool (dialog values baked in, the raw drag suppressed), the runner
builds the dormer with no dialog, `M`/`roofdormer` select the tool;
`docs/macro_language.md` updated. **AND HIS LOOK AT THE MERGED DORMER,
fixed on this branch too (2026‑09‑24):** *"There is a missing control
knob on the ridge of the dormer where it meets the roof. In addition
the walls of the dormer are being drawn down from the dormer. It should
show only the roof line and intersect with the walls under it."* — the
back-end grip is back and sets the RIDGE HEIGHT from the host plane at
the dragged point (the derived back end lands on it); the dormer's own
cheeks and face are gone from plan and 3D, and the house walls under a
dormer's territory climb into its roof (`_wall_under_roofs`, piers and
headers only). [`0194-report.md`](handoff/0194-report.md). His own direct
instruction as authority (`0065` §2, quoted in the reports); the reports
land on `main`, not here. His check: record a dormer in the recorder, replay the text on
a fresh plan, delete a host and undo it. Then R6 multifloor, on his
fixture and his display-rule answer ([`0186`](handoff/0186-ruling.md)
§4). his check. Then R6 multifloor, on his fixture and his display-rule answer
