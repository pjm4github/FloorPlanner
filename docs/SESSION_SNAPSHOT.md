...
**R5a IS CLOSED AND MERGED — [PR #61](https://github.com/pjm4github/FloorPlanner/pull/61)
landed on `main` by this merge commit on Patrick's word ("OK the dashed
lines look good"), the branch deleted in the merge step; report
[`0187-report.md`](handoff/0187-report.md).** Each roof draws a dashed
trace (`roofs.roof_clip_trace`, the R3b wall dash's own `ROOF_CLIP_INK`)
where its surface crosses the covered rooms' ceiling plane, cut per
room, the lower room governing over a shared wall, cut to R4d's visible
region while clipped. **HIS NEXT INSTRUCTION, given with that check and
NOT YET BUILT:** *"The only minor change we need is to clip the walls to
the roof when viewing in 3D. At the moment the walls stick up through
the roof."* — a `viewer/fp3d.py` change (the wall prisms capped at the
roof surface where it is lower than the wall top), his own direct
instruction as authority (`0065` §2, quoted in the report that builds
it), AMBER: its own branch and PR, stopped for his 3D look. Then, per
[`0186`](handoff/0186-ruling.md) §5: the R5b dormer read-back (a
report, no code — RED until he rules on it), R5b's build, R6 multifloor
on his fixture and his display-rule answer — never two open AMBER
tranches at once. 0187 §4 names the `fixtures/incoming/` duplicates (the
two promoted `w7` files, 23 days, unlisted since 0182) — exit 2 on his
word.
