...
**R4g — [`0181-ruling.md`](handoff/0181-ruling.md) JUST LANDED, ORDERS
ONE MORE PASS, NOT YET BUILT. Read it before doing anything else in
this area — it supplies the exact criterion 0178/0180's own diagnosis
was missing.** Branch `roofs-r4g-full-footprint` (off
`roofs-r4f-multi-ridge`), PR #60, open, AMBER, still stopped for
Patrick's check (which follows the 0% receipt this ruling orders, not
before). PR #59 already closed in favour of #60.
...
walking orphan fallback with a genuine prune-and-re-envelope fixed
point and fixed a real reach bug alongside it (both endorsed,
unchanged); 0180 measured the result (gap ratio on
`fixtures/threeRidgeFloorplan.json`: 27% → 15%, not the ordered 0%) and
diagnosed WHY, precisely, without yet fixing it: the anchor rule
certifies only the BIGGEST connected component of a roof's own single-
coverage ground, so a roof whose real construction genuinely splits
into two or more separate components (rf3 on this fixture; in
miniature, the equal-height-L's own outer B-corner) only gets one
certified — and two attempted fixes (validate every root component;
validate above a size threshold) both reopen the D85 regression
outright, measured, not assumed.

**0181's ruling, in full, is the criterion: a roof may connect to its
own anchor by passing UNDER a piece another roof currently draws (its
own surface strictly BELOW the drawn one there — a real, physical
"runs underneath and comes out the other side"), but NEVER by passing
ABOVE one (its own surface higher than the drawn one — a phantom
floating in the air, exactly what the poke-through fix already forbids
and exactly D85's corner's own only route to A's body).** Build a
"continuity network" per roof: its own drawn pieces, plus every piece
where it lies strictly under whoever IS drawn there; a root component
certifies iff it connects to the roof's principal (biggest) anchor
component THROUGH that network. D85's corner stays blank (its only
route is through the poke-through zone, which is above, not under); the
L's outer-corner B scrap and rf3's rake-corner component both certify
(their routes are under, not above) — restoring, per the ruling, the
outer-corner seam-vertex assertion `0180-report.md` §4 dropped, not
merely leaving it dropped. Expected result: 0% blank on the three-ridge
fixture (partition invariant restored as *blank ⊆ root ground of
uncertified limbs*, and that set is empty here), and on the equal-
height-L. D85, the poke-through fix, the pinch exclusion, 0 interior
jumps, and the T/L suites are the floor — 0180 already proved they
survive with everything except the wrong fallback, and none of them are
negotiable through this pass either. If the network rule fails a floor
test anywhere, STOP and report the failing geometry — this is meant to
be the fourth and last shape, not a fourth guess.
