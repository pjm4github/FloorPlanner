...
| | |
|---|---|
| **`main`** | **`f0e0d83`** at this file's cut. PR #39/#37/#38/#40/#41/#42/#43/#44/#45 all merged. **`0108`-`0110`'s three snap-to-grid features COMPLETE.** **Still open, Patrick's alone, no code blocks it: the `L2.dxf` Chief recount** (`0066` §6's closing receipt for item C, **still the oldest open line on the board**). **THE CLEANUP TRANCHE — BUILT, GATE GREEN, PR PENDING on `dimension-cleanup-tranche`**, per [`0129-ruling.md`](handoff/0129-ruling.md) §3 / [`0130-ruling.md`](handoff/0130-ruling.md) / [`0132-ruling.md`](handoff/0132-ruling.md): (a) grid-aware station filtering, reading the doc's own `wall_snap_in` (`Sheet.wall_snap_in`, defaulting 6.0); (b) lane labels confirmed already feet-and-inches; (c) title-block note → "wall centerlines"; (d) openings already out, reaffirmed; **plus `0130`'s family exclusivity — each row/lane dimensions only walls at its own angle, no cross-family leakage, per-family telescoping (automatic once `_features()` is scoped correctly).** Check file `wiscaway2026-08-30R2.json`/`R2g.pdf` promoted from `incoming/`, superseding R1. Full trail: [`handoff/0134-report.md`](handoff/0134-report.md), `handoff/README.md`'s pair table. **Numbering collisions, on the record:** `0036`, `0043`, `0050`, `0101`; neither renamed, numbering continues forward each time. |
| **Branches** | **`dimension-cleanup-tranche`** — AMBER, PR pending, `0129`/`0130-ruling.md`'s grid filter + family exclusivity, stopped for Patrick's check (export Wiscaway once more). **`wall-orthogonality-repair`** ([PR #37](https://github.com/pjm4github/FloorPlanner/pull/37), item C) — AMBER, open, **check UNBLOCKED, ready to run** (against `wiscaway2026-08-09R.json`). **`wall-report-id-fix`** ([PR #39](https://github.com/pjm4github/FloorPlanner/pull/39), the standalone report's own fix) — AMBER, open, ready. **`wall-label-angle-clause`** ([PR #38](https://github.com/pjm4github/FloorPlanner/pull/38), the angle clause) — AMBER, open, worth checking, unaffected. `angled-dimension-lanes`/`dim-row-along-refactor`/`pdf-dimension-and-door-fix`/`export-menu-pdf` merged and deleted, local and remote. `fp2dxf-integration` deleted earlier, joining the five from [`0053`](handoff/0053-ruling.md) §2 item 4. |
