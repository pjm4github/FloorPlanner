...
| | |
|---|---|
| **`main`** | **`904bd93`** at this file's cut — this snapshot's marker (line 1), required to match this row exactly; `main` itself is ahead (see `handoff/README.md`'s pair table). This copy is carried onto `cross-floor-align-fix` while bringing that branch current before its merge, per `0090-ruling.md`. PR #36 (`t-junction-grid-snap`, D80's fix) MERGED — re-gated GREEN on the combined tree first, real two-parent merge commit, branch deleted local+remote. **[`0090-ruling.md`](handoff/0090-ruling.md) is the current status** — read that first: all three checks PASSED (`0088`/`0089`/`0090`), the AMBER queue that was the bottleneck is EMPTY. #34 and #35 merge next, in that order, each re-gated on the combined tree first. `wall-orthogonality-repair` (PR #37) becomes the only AMBER item once they land — `0084-ruling.md` §6 items 1-3 (restore `T`, the post-condition, re-measure) are owed first. Full trail: `handoff/README.md`'s pair table. |
| **Branches** | **`cross-floor-align-fix`** (#34) **· `wall-label-fixes`** (#35) — **both PASSED Patrick's check, merging next, in that order** (`0090`). `t-junction-grid-snap` (#36) MERGED, deleted. **`wall-orthogonality-repair`** ([PR #37](https://github.com/pjm4github/FloorPlanner/pull/37), item C) is AMBER, not yet ready for a check — `0084-ruling.md` §6 items 1-3 owed first. `fp2dxf-integration` deleted, local and remote, joining the five from [`0053`](handoff/0053-ruling.md) §2 item 4. |
