...
**THE 3D WALL CLIP — Patrick's own instruction on R5a's check, 2026‑09‑13
(*"clip the walls to the roof when viewing in 3D. At the moment the
walls stick up through the roof"*) — BUILT ON THIS BRANCH
(`roofs-3d-wall-clip`), PR #62, AMBER — HIS FIRST LOOK FOUND ONE THING,
FIXED ON THE BRANCH, STOPPED FOR HIS SECOND:** *"that looks pretty good
... We need to move the walls down just a little so that the wall doesnt
poke through the roof in 3D view at some angles"* — the cap sat ON the
roof's top surface, two coplanar faces fighting; it now sits
`WALL_CAP_BELOW_ROOF_IN` (the slab's 4″ plus ½″ clearance) under it
([`0189-report.md`](handoff/0189-report.md), on `main`). `viewer/fp3d.py`:
the roof clips are computed BEFORE the walls now, and every solid piece
of a wall (`_wall_under_roofs`) is capped at the surface of the roof
whose territory (R4d/R4g's visible region; a lone roof's footprint) it
lies under — the quad split by the territory's cell edges and the roof's
own plane-change lines so each cap is one exact plane; a piece the roof
...
clear their wall tops builds byte-identically to before. Report
[`0188-report.md`](handoff/0188-report.md) lands on `main`, not on this
branch. **R5a IS CLOSED AND MERGED** — [PR #61](https://github.com/pjm4github/FloorPlanner/pull/61)
on his word ("OK the dashed lines look good"), [`0187-report.md`](handoff/0187-report.md).
Then, per [`0186`](handoff/0186-ruling.md) §5: the R5b dormer read-back
(a report, no code — RED until he rules on it), R5b's build, R6
multifloor on his fixture and his display-rule answer — never two open
AMBER tranches at once. 0187 §4 names the `fixtures/incoming/` duplicates
(the two promoted `w7` files, 23 days, unlisted since 0182) — exit 2 on
his word.
