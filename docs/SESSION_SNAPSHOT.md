...
both; D50 CLOSED (`closed_by b8475b7`). **NOTHING IS OWED BY CODE. Next
is his one gesture (0197 §3): open `fixtures/wiscaway2026-08-30R2.json`
(or his own current wiscaway), set L2's elevation to the real storey
height, draw a roof on L2 over the upper walls, save, and drop it in
`fixtures/incoming/` — that file is the R6 fixture.** Then R6.a (0197
§4's two answers: this level's roofs solid, other levels' covering
roofs ghosted; the 3D view defaults to the whole building) → R6.b
