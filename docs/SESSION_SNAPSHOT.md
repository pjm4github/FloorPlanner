...
| | |
|---|---|
| **`main`** | **`a86d419`** at this file's cut. PR #39/#37/#38/#40/#41/#42/#43/#44 all merged. **`0108`-`0110`'s three snap-to-grid features COMPLETE.** **Still open, Patrick's alone, no code blocks it: the `L2.dxf` Chief recount** (`0066` §6's closing receipt for item C). **D81/D82 CLOSED** — full trail: [`handoff/0121-report.md`](handoff/0121-report.md), [`0122-report.md`](handoff/0122-report.md). **TRANCHE 2 (aligned dimensions for angled walls), per [`0123-ruling.md`](handoff/0123-ruling.md):** **1. `dim_row_along` refactor MERGED** — [PR #44](https://github.com/pjm4github/FloorPlanner/pull/44) → `main` at `a86d419`, GREEN, no check owed. **2. The room-`show_dimensions`-driven angled lane** ([`0120`](handoff/0120-ruling.md) §2) — AMBER, BUILDING NOW on top of the merged refactor, own branch, stops for Patrick's check. Full trail: `handoff/README.md`'s pair table. **Numbering collisions, on the record:** `0036`, `0043`, `0050`, `0101`; neither renamed, numbering continues forward each time. |
