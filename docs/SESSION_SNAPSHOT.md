...
**R5b DORMERS — [`0191-ruling.md`](handoff/0191-ruling.md) adopted
[`0190`](handoff/0190-report.md)'s read-back with one correction (a
separate Dormer tool) — BUILT, AMBER, STOPPED FOR PATRICK'S CHECK:
[PR #63](https://github.com/pjm4github/FloorPlanner/pull/63) on
`roofs-r5b-dormers`, report [`0192-report.md`](handoff/0192-report.md)
(this commit lands it, with his ruling).** A dormer is a `roof` record
with an optional `host` (schema, `Roof`, writer, loader with a
dangling-host skip, `canonicalize` remapping the reference); its back end
is derived where the ridge meets the host plane (`roofclip.meet_along`)
and still written; cheeks and face are derived plan lines and, in
`fp3d`, prisms standing on the host plane; `roof_clip_spans` reads a
roof's drawn territory; the Dormer tool is its own Roof-menu item,
toolbar button and macro letter `M` (press on a roof plane snapped to
the R5a trace and the grid along it, drag along it for width, Shift
frees the direction, the End-On dialog refuses a ridge the host never
meets); `fixtures/dormer-gable-check.json` promoted. 0192 §5 owns a
correction to 0190 §0's second measurement (that wall was not under the
dormer; the fix is exercised with the face beyond the trace). His check
(0191 §3): sketch a dormer on the loft from the Dormer tool — the trace
and the room's dashes open under it, the valleys draw solid, the back
end lands on the host plane by itself — then the same in 3D. **On his
word: merge PR #63, delete the branch in the merge step.** The two
`w7` duplicates in `fixtures/incoming/` are deleted in this commit
(0191 §4, exit 2). Then R6 multifloor, on his fixture and his
display-rule answer ([`0186`](handoff/0186-ruling.md) §4). **THE 3D
WALL CLIP IS CLOSED AND MERGED** ([PR #62](https://github.com/pjm4github/FloorPlanner/pull/62),
