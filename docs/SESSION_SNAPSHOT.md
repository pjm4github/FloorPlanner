...
AND MADE D50 ITS GATE — R6.0 BUILT ON THIS BRANCH (`roofs-r6-0-d50`),
AMBER, STOPPED FOR HIS CHECK:** the loader was measured first (it
...
both, the loader and the three writers (scene walk, detect, legacy
import) plus the v4 project path read them; a new floor defaults to the
floor below's elevation plus its height, written not derived; Floors ▸
<floor> ▸ "Elevation and height…" edits both (an undo step); D50's
receipt across all five multifloor plans as tests; D50 CLOSED on this
branch (`closed_by` = the fix commit). Measured for 0197 §6: a stored
elevation stacks the storeys in 3D by itself — `fp3d` reads
`elevation_in` as the level base and `height_in` as its wall top. Report
[`0198-report.md`](handoff/0198-report.md) lands on `main`, not here.
His check (0197 §2 item 6): open a multifloor plan, set L2's elevation
and height, save, reopen — unchanged; then §3, his one gesture: open
`fixtures/wiscaway2026-08-30R2.json`, set L2's elevation to the real
storey height, draw a roof on L2, save — that file is the R6 fixture.
Then R6.a (0197 §4's two answers) → R6.b → D67 reproduced or refuted →
R6.c.**
