...
| | |
|---|---|
| **`main`** | **`890fafb`** at this file's cut. PR #39/#37/#38/#40/#41/#42/#43/#44/#45/#46/#47 all merged. **`0108`-`0110`'s three snap-to-grid features COMPLETE.** **Still open, Patrick's alone, no code blocks it: the `L2.dxf` Chief recount** (`0066` §6/§7 — **the oldest and now the ONLY open line on the board**; the repair tool is already live on `main`, purely his own manual step, no code). **PDF cleanup tranche MERGED** ([PR #46](https://github.com/pjm4github/FloorPlanner/pull/46), `0134-report.md`). **`fp2dxf`'s door-symbol-by-type + real windows PASSED CHECK, MERGED — [PR #47](https://github.com/pjm4github/FloorPlanner/pull/47) → `main` at `890fafb`** — Patrick's own words: "The DXF export works as expected." Full trail: [`handoff/0136-report.md`](handoff/0136-report.md), `handoff/README.md`'s pair table. **Numbering collisions, on the record:** `0036`, `0043`, `0050`, `0101`; neither renamed, numbering continues forward each time. |
