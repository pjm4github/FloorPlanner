...
walls stick up through the roof"*) — BUILT, [PR #62](https://github.com/pjm4github/FloorPlanner/pull/62)
on `roofs-3d-wall-clip`, [`0188-report.md`](handoff/0188-report.md);
HIS FIRST LOOK FOUND ONE THING, FIXED ON THE BRANCH, AMBER, STOPPED FOR
HIS SECOND — [`0189-report.md`](handoff/0189-report.md), which this
commit lands:** *"that looks pretty good … We need to move the walls
down just a little so that the wall doesnt poke through the roof in 3D
view at some angles"* — the cap sat ON the roof's top surface, two
coplanar faces fighting; it now sits `WALL_CAP_BELOW_ROOF_IN` (the
slab's 4″ plus ½″ clearance) under it.** `viewer/fp3d.py`: the roof clips are computed
