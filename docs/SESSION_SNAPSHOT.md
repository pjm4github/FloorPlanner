...
| | |
|---|---|
| **`main`** | **`8fb3404`** at this file's cut. PR #39/#37/#38/#40/#41/#42 all merged. **`0108`-`0110`'s three snap-to-grid features COMPLETE.** **Still open, Patrick's alone, no code blocks it: the `L2.dxf` Chief recount** (`0066` §6's closing receipt for item C). **`0116-ruling.md`'s Export menu — BUILT, CHECKED, MERGED — [PR #42](https://github.com/pjm4github/FloorPlanner/pull/42) → `main` at `8fb3404`.** Real `Export` submenu (Rooms as CSV / Chief Architect DXF / PDF plan set / Legacy v4), the literal `▸` removed from the DXF label, PDF wired to `fp2pdf.py` with an options dialog + Save-As, `reportlab`-missing disabled-not-crashed. **Patrick's own manual check passed the menu shape and the PDF Save-As pipeline** and authorised the merge; it also found two PDF-*content* defects, filed separately rather than blocking the merge — [D81](defects/0081-fp2pdf-draws-every-door-as-the-same-generic.md) (every door renders as the same generic swing leaf; `door_type` is never read correctly) and [D82](defects/0082-fp2pdf-dimension-strings-carry-1-16-fractions.md) (dimension strings carry a 1/16" fraction; wanted feet-inches only) — both open, not built. Full trail: [`handoff/0117-report.md`](handoff/0117-report.md), `handoff/README.md`'s pair table. **Numbering collisions, on the record:** `0036`, `0043`, `0050`, `0101`; neither renamed, numbering continues forward each time. |
| **Branches** | **`wall-orthogonality-repair`** ([PR #37](https://github.com/pjm4github/FloorPlanner/pull/37), item C) — AMBER, open, **check UNBLOCKED, ready to run** (against `wiscaway2026-08-09R.json`). **`wall-report-id-fix`** ([PR #39](https://github.com/pjm4github/FloorPlanner/pull/39), the standalone report's own fix) — AMBER, open, ready. **`wall-label-angle-clause`** ([PR #38](https://github.com/pjm4github/FloorPlanner/pull/38), the angle clause) — AMBER, open, worth checking, unaffected. `export-menu-pdf` merged, not yet deleted (remote branch kept per `--delete-branch=false`; safe to delete once Patrick has no further reason to compare against it). `fp2dxf-integration` deleted, local and remote, joining the five from [`0053`](handoff/0053-ruling.md) §2 item 4. |
