...
| **Records** | **55 records — 15 open, 40 closed.** `python tools/gate.py --docs` **GREEN** (front matter valid, `INDEX.md` matches a regeneration, every defect reference resolves, migration dry run clean). `Ref-Audit: unresolved=10` — dangling *links and paths*, which are reported and never enforced; dangling *keys* would fail. |
