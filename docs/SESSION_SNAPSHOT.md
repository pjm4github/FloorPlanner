...
| | |
|---|---|
| **`main`** | **`f5dbffa`** at this file's cut. PR #39/#37/#38/#40/#41/#42/#43/#44/#45 all merged. **`0108`-`0110`'s three snap-to-grid features COMPLETE.** **Still open, Patrick's alone, no code blocks it: the `L2.dxf` Chief recount** (`0066` §6's closing receipt for item C, **still the oldest open line on the board** — `0129` §4's own words; the cleanup export session below could close it). **TRANCHE 2's angled-dimension-lane PASSED CHECK, MERGED — [PR #45](https://github.com/pjm4github/FloorPlanner/pull/45) → `main` at `abe5e6e`.** Three fully-merged ghost branches deleted (`dim-row-along-refactor`, `export-menu-pdf`, `pdf-dimension-and-door-fix`) — full trail: [`handoff/0133-report.md`](handoff/0133-report.md). **THE CLEANUP TRANCHE — BUILDING NOW, one branch, one check**, per [`0129-ruling.md`](handoff/0129-ruling.md) §3 / [`0130-ruling.md`](handoff/0130-ruling.md) / [`0132-ruling.md`](handoff/0132-ruling.md): (a) grid-aware station filtering reading the doc's own `wall_snap_in`; (b) lane labels already feet-and-inches, verify; (c) title-block note → "wall centerlines"; (d) openings already out, reaffirmed; **plus `0130`'s amendment — each row/lane dimensions only walls at its own angle, no cross-family leakage, per-family telescoping.** Check file: `wiscaway2026-08-30R2.json`/`R2g.pdf` (0132, two levels, supersedes R1 as the baseline). Full trail: `handoff/README.md`'s pair table. **Numbering collisions, on the record:** `0036`, `0043`, `0050`, `0101`; neither renamed, numbering continues forward each time. |
| **Branches** | **`wall-orthogonality-repair`** ([PR #37](https://github.com/pjm4github/FloorPlanner/pull/37), item C) — AMBER, open, **check UNBLOCKED, ready to run** (against `wiscaway2026-08-09R.json`). **`wall-report-id-fix`** ([PR #39](https://github.com/pjm4github/FloorPlanner/pull/39), the standalone report's own fix) — AMBER, open, ready. **`wall-label-angle-clause`** ([PR #38](https://github.com/pjm4github/FloorPlanner/pull/38), the angle clause) — AMBER, open, worth checking, unaffected. `angled-dimension-lanes` merged and deleted (`--delete-branch`, per `0129` §1's own procedure fix — deletion now folds into the merge step, not a separate one that lapses). `dim-row-along-refactor`/`pdf-dimension-and-door-fix`/`export-menu-pdf` — the three ghosts (`0129` §2) — deleted, local and remote. `fp2dxf-integration` deleted earlier, joining the five from [`0053`](handoff/0053-ruling.md) §2 item 4. |
