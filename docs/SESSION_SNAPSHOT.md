...
| | |
|---|---|
| **`main`** | **`2c75096`** at this file's cut. PR #39/#37/#38/#40/#41/#42/#43/#44/#45/#46 all merged. **`0108`-`0110`'s three snap-to-grid features COMPLETE.** **Still open, Patrick's alone, no code blocks it: the `L2.dxf` Chief recount** (`0066` §6's closing receipt for item C, **still the oldest open line on the board**). **THE CLEANUP TRANCHE PASSED CHECK, MERGED — [PR #46](https://github.com/pjm4github/FloorPlanner/pull/46) → `main` at `bb9309a`**, per [`0129-ruling.md`](handoff/0129-ruling.md) §3 / [`0130-ruling.md`](handoff/0130-ruling.md) / [`0132-ruling.md`](handoff/0132-ruling.md): (a) grid-aware station filtering reading the doc's own `wall_snap_in`; (b) lane labels confirmed feet-and-inches; (c) title-block note → "wall centerlines"; (d) openings reaffirmed out; (e) `0130`'s family exclusivity — each row/lane dimensions only walls at its own angle, per-family telescoping. `wiscaway2026-08-30R2.json`/`R2g.pdf` promoted, superseding R1. Full trail: [`handoff/0134-report.md`](handoff/0134-report.md), `handoff/README.md`'s pair table. **Numbering collisions, on the record:** `0036`, `0043`, `0050`, `0101`; neither renamed, numbering continues forward each time. |
| **Branches** | **CORRECTION, found 2026‑08‑31: [PR #37](https://github.com/pjm4github/FloorPlanner/pull/37)/[#38](https://github.com/pjm4github/FloorPlanner/pull/38)/[#39](https://github.com/pjm4github/FloorPlanner/pull/39) were already MERGED on 2026‑08‑26** — this row had called them "AMBER, open" for the entire session before this correction, stale from before this session's own first snapshot read. **Edit ▸ "Repair wall orthogonality…" (item C) is already live on `main`** (`mainwindow.py:368`) — nothing to build or merge for it. What `0066-ruling.md` §7 actually leaves open is **Patrick's own manual step alone**: open the plan that produced `L2.dxf`, run the repair, export via File ▸ Export ▸ Chief Architect (DXF)…, open in Chief Architect X17, count flagged walls against the 75 baseline, confirm the drawing still looks right. No branch, no PR, no code. `dimension-cleanup-tranche`/`angled-dimension-lanes`/`dim-row-along-refactor`/`pdf-dimension-and-door-fix`/`export-menu-pdf` merged and deleted, local and remote. `fp2dxf-integration` deleted earlier, joining the five from [`0053`](handoff/0053-ruling.md) §2 item 4. |
