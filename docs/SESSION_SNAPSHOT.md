...
its roof). THEN [`0195-ruling.md`](handoff/0195-ruling.md) — 0194 adopted on
measurement with one overclaim corrected and one demand — BUILT, AMBER,
STOPPED FOR HIS CHECK: [PR #65](https://github.com/pjm4github/FloorPlanner/pull/65)
on `roofs-r5e-grip-clamp`, report [`0196-report.md`](handoff/0196-report.md)
(this commit lands it, with his ruling; 0196 opens with the record of
PR #64's merge that 0194 owed).** The dormer's back-end drag is clamped
to the RISING side of the host's surface inside its footprint
(`roofclip.rising_reach`; past the crest the grip stops dead instead of
folding back), two fail-first tests plus the pinned rising side; the
closure below the dormer's eaves is NAMED (R4f's `_cross_roof_risers`
along the unequal-height boundaries) and PROBED (rays through both
eaves planes and the front hit; with the risers removed they miss);
0194 §1's overclaim corrected. His check: drag the dormer's back knob
past the host's crest. **On his word: merge PR #65, delete the branch
in the merge step.** Then R6 multifloor, waiting on his fixture and his
one-line display-rule answer ([`0186`](handoff/0186-ruling.md) §4).**
