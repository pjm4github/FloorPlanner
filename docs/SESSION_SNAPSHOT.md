...
| | |
|---|---|
| **`main`** | **`36fb6b5`** at this file's cut. PR #39/#37/#38/#40/#41/#42/#43 all merged. **`0108`-`0110`'s three snap-to-grid features COMPLETE.** **Still open, Patrick's alone, no code blocks it: the `L2.dxf` Chief recount** (`0066` §6's closing receipt for item C). **D81/D82 — the PDF exporter's door symbols and dimension pile-up — BUILT, CHECKED, MERGED — [PR #43](https://github.com/pjm4github/FloorPlanner/pull/43) → `main` at `36fb6b5`, both defects CLOSED.** Station clustering + whole-inch telescoping labels (D82) and one PDF symbol per catalog `door_type` (D81); corpus receipt `docs/evidence/pdf_dimension_telescoping_census.py` (964→856 stations, `wiscaway2026-08-09R.json` 190→156). Full trail: [`handoff/0121-report.md`](handoff/0121-report.md), [`0122-report.md`](handoff/0122-report.md). **Patrick's own follow-up, same check: the 45° walls are not labeled correctly in the PDF — confirmed as `0119`/`0120`'s own already-measured gap, queued as its own focused effort next**, not a new finding. `0119`/`0120` (tranche 2, aligned dimension lanes for angled walls) on disk; `0120` (Patrick's room-`show_dimensions`-driven scheme) supersedes `0119` §2's per-run placement, `0119` §1/§3 stand. **Next, in order:** `0119` §3's `dim_row_along` refactor (GREEN), then `0120`'s angled-lane feature (AMBER, own branch) — nothing blocks starting, `0118`'s station machinery both depend on is merged. Full trail: `handoff/README.md`'s pair table. **Numbering collisions, on the record:** `0036`, `0043`, `0050`, `0101`; neither renamed, numbering continues forward each time. |
| **Branches** | **`wall-orthogonality-repair`** ([PR #37](https://github.com/pjm4github/FloorPlanner/pull/37), item C) — AMBER, open, **check UNBLOCKED, ready to run** (against `wiscaway2026-08-09R.json`). **`wall-report-id-fix`** ([PR #39](https://github.com/pjm4github/FloorPlanner/pull/39), the standalone report's own fix) — AMBER, open, ready. **`wall-label-angle-clause`** ([PR #38](https://github.com/pjm4github/FloorPlanner/pull/38), the angle clause) — AMBER, open, worth checking, unaffected. `pdf-dimension-and-door-fix` merged, not yet deleted (remote branch kept per `--delete-branch=false`). `export-menu-pdf` merged, not yet deleted, same reason. `fp2dxf-integration` deleted, local and remote, joining the five from [`0053`](handoff/0053-ruling.md) §2 item 4. |
