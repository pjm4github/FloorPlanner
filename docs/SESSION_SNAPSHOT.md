...
§2 — BUILT ON THIS BRANCH (`roofs-r5a-clip-trace`), AMBER, STOPPED FOR
PATRICK'S CHECK.** With the roof layer shown, each roof draws a dashed
trace (the R3b wall dash's own orange, now `ROOF_CLIP_INK` in
`config.py`) along the locus where its surface crosses the covered
rooms' ceiling plane — `roofs.roof_clip_trace`, the same `roof_clip_spans`
geometry read from the roof's side: one line per side parallel to the
ridge, a cross segment past a hip end, cut per room so a taller room
carries its own line and the LOWER room governs over a shared wall
exactly as the wall dash does; drawn only inside R4d's visible region
while clipped. His check, per the ruling: wiscaway with roofs on, the
trace hugging where the R3b wall dashes already end. Evidence:
`docs/evidence/roof-clip-trace-r5a.png` (two rooms, 96″ and 120″, a hip
end). Report [`0187-report.md`](handoff/0187-report.md) lands on `main`,
not on this branch. Then, per [`0186`](handoff/0186-ruling.md) §5: the
R5b dormer read-back (RED until ruled), R5b's build, R6 multifloor on
his fixture — never two open AMBER tranches at once. R4g's close:
[`0185-report.md`](handoff/0185-report.md); 0186 §1 ratified crossed
arms stay joined and the ridge-end-point rule as a named limit.
