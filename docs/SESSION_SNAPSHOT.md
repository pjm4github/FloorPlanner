...
| | |
|---|---|
| **`main`** | **`b097bbb`** at this file's cut — `31300e8` landed `floorplanner/export/fp2pdf.py` (Patrick's new PDF exporter, ruff-only fixes, not yet wired in), `b097bbb` landed `0070`-`0073`. This commit lands `0074`/`0075` too: a persistent app-settings file, on Patrick's own follow-up instructions — **JSON, not `QSettings`** (`0075`: a stdlib-typed-hazard rejected, not relocated — `configparser` would keep the identical bug), a three-rung precedence chain (open document > settings file > code default) so a saved plan always reopens as saved, and a named INI→JSON migration-ordering trap that would otherwise silently drop a real user's saved API key. **All four rulings (`0070`-`0075` minus D80) are RED/AMBER-heavy with a few GREEN steps not yet built** — none of `0072`-`0075`'s code has been written. Full trail: `handoff/README.md`'s pair table. |
