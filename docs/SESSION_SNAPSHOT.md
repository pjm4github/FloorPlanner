...
| | |
|---|---|
| **`main`** | **`8e81d2b`** at this file's cut — `31300e8`/`b097bbb` landed `fp2pdf.py` and `0070`-`0073`, `8e81d2b` landed `0074`/`0075` (the settings-file rulings). **This commit builds every GREEN item across `0072`-`0075`** — [`0076-report.md`](handoff/0076-report.md): `coerce_setting()` (one function, both settings loaders); the app-settings store rebuilt on plain JSON, `QSettings` dropped entirely, the string/`False`-reads-`True` hazard receipted directly; INI→JSON migration with its own ordering-trap receipt; `fp2pdf.py`'s four hygiene faults fixed to match `fp2dxf.py`'s own precedent. 23 new tests. **Still AMBER/RED and not built**: new settings keys, the three-rung chain, the checkbox, the export menu, the PDF dialog, `--settings`. Full trail: `handoff/README.md`'s pair table. |
