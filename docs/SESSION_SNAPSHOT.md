...
| | |
|---|---|
| **`main`** | **`142699b`** at this file's cut. **PR #39 and PR #37 (item C, the wall orthogonality repair) BOTH MERGED** — Patrick's checks passed on both (PR #39: report rows name a findable wall, click centres + selects; PR #37: `wiscaway2026-08-09R.json`, 5 walls offered, largest correction 0.041″, applied, drawing still correct). `floorplanner/dialogs.py`'s `WallRowList` class merged cleanly with no duplicate — both branches had inserted byte-identical code. **`0108`-`0110` spec three new AMBER features** (queued behind PR #38, per `0111`): "Snap to Grid" (per-wall, manual, AMBER), "Snap to Grid Orthogonal" (the safer variant, build **first**), a disabled placeholder menu item for the 15°-angle case (GREEN, no arithmetic exists for it — `tan 15°` is irrational, proven in `0110` §3). **`0111`/`0112`'s close-out, IN PROGRESS**: PR #38 next and last — its own check already passed (Ctrl-drag/Shift-drag angle clause), merge and delete branch, then the queue reopens. **Numbering collisions, on the record:** `0036`, `0043`, `0050`, `0101`; neither renamed, numbering continues forward each time. |
