...
**R4g — [`0181-ruling.md`](handoff/0181-ruling.md) BUILT, [`0182-report.md`](handoff/0182-report.md);
HIS CHECK PASSED THE 3D VIEW ("That looks fantastic!") AND FOUND A
MISSING JOINT IN THE PLAN, FIXED — [`0183-report.md`](handoff/0183-report.md):
seams are now read off the final pieces (every equal-height shared edge
between two roofs), not off the in-round crossing bookkeeping the
per-piece prune had bypassed; his re-check passed ("The joints look
good"). THEN HIS SECOND FIXTURE, `fixtures/threeRidgeShortRidgeFloorplan.json`
(rf2's ridge cut short over rf1) — [`0184-report.md`](handoff/0184-report.md):
a swallowed end that stops short of every host's ridge, standing above
the host, is OPEN (a gable end: no extension, end line drawn, fascia in
3D); an end that has crossed a host's ridge is a cross-gable's arm and
never open (the pinch fixture byte-for-byte unchanged). HIS CHECK PASSED
("that looks absolutely perfect!", both views) — R4g CLOSED, MERGED to
`main` on that word by this very merge commit, the branch deleted in
the merge step.** Branch `roofs-r4g-full-footprint` (off `roofs-r4f-multi-ridge`,
`main` at `7714ae7` merged into it), PR #60, open. PR #59 already closed
in favour of #60. His check: the 3D view of
`fixtures/threeRidgeFloorplan.json` and of the equal-height L, and
`docs/evidence/threeridge-drawn-junction-0182.png` against his own v2
reference picture (`threeridge-reference-junction-v2.png`).
...
original coverers, blank cells, limb cells, and every round.
