...
| | |
|---|---|
| **`main`** | **`79f5ef3`** at this file's cut. PR #39/#37/#38/#40/#41/#42 all merged. **`0108`-`0110`'s three snap-to-grid features COMPLETE.** **Still open, Patrick's alone, no code blocks it: the `L2.dxf` Chief recount** (`0066` §6's closing receipt for item C). **D81/D82 — the PDF exporter's door symbols and dimension pile-up — BUILT, GATE GREEN, PR PENDING on `pdf-dimension-and-door-fix`**, per [`0118-ruling.md`](handoff/0118-ruling.md): station clustering + whole-inch telescoping labels (closes D82, the "mess of dimensions on top of each other" — corpus receipt `docs/evidence/pdf_dimension_telescoping_census.py`, 964→856 stations, `wiscaway2026-08-09R.json` 190→156) and one PDF symbol per catalog `door_type` (closes D81). Full trail: [`handoff/0121-report.md`](handoff/0121-report.md). **`0119-ruling.md`/`0120-ruling.md` landed alongside** — tranche 2, aligned dimension lanes for angled walls; `0120` (Patrick's room-`show_dimensions`-driven scheme) supersedes `0119` §2's per-run placement, `0119` §1/§3 stand. **Queued next, in order, once `0118` merges** (its station machinery is the base): `0119` §3's `dim_row_along` refactor (GREEN), then `0120`'s angled-lane feature (AMBER, own branch). Full trail: `handoff/README.md`'s pair table. **Numbering collisions, on the record:** `0036`, `0043`, `0050`, `0101`; neither renamed, numbering continues forward each time. |
| **Branches** | **`pdf-dimension-and-door-fix`** — AMBER, PR pending, D81/D82 (`0118-ruling.md`), stopped for Patrick's check (export `wiscaway2026-08-09R`: whole-inch dimensions that sum, French/pocket doors that look like French/pocket doors). **`wall-orthogonality-repair`** ([PR #37](https://github.com/pjm4github/FloorPlanner/pull/37), item C) — AMBER, open, **check UNBLOCKED, ready to run** (against `wiscaway2026-08-09R.json`). **`wall-report-id-fix`** ([PR #39](https://github.com/pjm4github/FloorPlanner/pull/39), the standalone report's own fix) — AMBER, open, ready. **`wall-label-angle-clause`** ([PR #38](https://github.com/pjm4github/FloorPlanner/pull/38), the angle clause) — AMBER, open, worth checking, unaffected. `export-menu-pdf` merged, not yet deleted (remote branch kept per `--delete-branch=false`; safe to delete once Patrick has no further reason to compare against it). `fp2dxf-integration` deleted, local and remote, joining the five from [`0053`](handoff/0053-ruling.md) §2 item 4. |
...
| **Records** | **83 records, 33 open.** D75 an accepted limit, D44's precedent; D76 the non-compositing renderer limit, cross-referenced to D69; D77 a tooling gap in `fp3d.py --shot`. D78 CLOSED (fixed 2026‑08‑16, `handoff/0027-ruling.md`). D80 CLOSED (fixed 2026‑08‑22, closed 2026‑08‑23 on Patrick's own check, `handoff/0088-ruling.md`, merged `main` at `ac6d763`). **D81/D82 OPEN, filed 2026‑08‑28** off PR #42's manual check — `fp2pdf.py`'s door symbols and dimension-fraction formatting, not built. `python tools/gate.py --docs` GREEN. |
