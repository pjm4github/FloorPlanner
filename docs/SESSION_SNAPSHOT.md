...
| | |
|---|---|
| **`main`** | **`751009b`** at this file's cut — this snapshot's marker (line 1), required to match this row exactly; `main` itself is ahead (see `handoff/README.md`'s pair table). This copy is carried onto `wall-orthogonality-repair` while bringing that branch current after `0084-ruling.md` §6 items 1-3 landed there. **[`0095-report.md`](handoff/0095-report.md) is the current status** — `T` restored as the candidacy filter, the orthogonality post-condition added (a real exemption bug in its own first draft found and fixed against `wiscaway`'s `w57`), the corpus census re-run: the whole-file rollback no longer fires at all (0 of 63, was 37 stranded). **PR #37 is worth checking now.** Also: `0068` §4 (heading vs. deviation) ANSWERED — `0092`/`0093`/`0094` rule "both numbers", suppressed on-intended-angle, AMBER, not yet built, batched with PR #37's check. `0085`'s `CLAUDE.md` trim is GREEN, unblocked, still owed. Full trail: `handoff/README.md`'s pair table. |
