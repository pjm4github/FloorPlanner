...
**A CONTROL PROVES THE QUESTION IT WAS BUILT TO ANSWER, AND NO MORE — added 2026‑08‑15, Patrick's ruling on [`handoff/0022-ruling.md`](handoff/0022-ruling.md), quoting Code's own sentence verbatim. Fourth member of this family** (the third — an instrument that can report only ONE of its two possible answers — is [`handoff/0018-ruling.md`](handoff/0018-ruling.md) §2's finding, on `0017`'s control).

The positive control catches an instrument reporting **nothing**. The identical-cases rule catches one reporting a **plausible something**. The third catches one that can only ever **say one of two things**. This one is a different shape again: the instrument was **correct** on the question it asked, and silent anyway — because the fault it needed to catch was not *in* that question.

**The instance.** `roof_over()` asks *is there a face at the body's full height over the region's centre* — a question about the **cap**. The first cut of the vessel/enclosure split reused the wrong extrusion formula and built `walk_in_shower`'s bench spanning **18″–78″**, a column hanging near the ceiling, instead of **0″–18″**, standing on the floor. The bench never touched the cap, so `roof_over()` returned exactly the answer it would have returned on a correctly-placed bench. **The classification was right and the construction was wrong, and one control cannot cover both** — the bug was outside the instrument's domain, not merely missed by it. It was caught instead by a cheaper, more direct instrument: dumping the built mesh's own bounding box.

> **Ask of any instrument, once it has passed: what question does it answer, and what does that question structurally not reach?** A construction error one layer below the question asked is invisible by design, not by oversight — and the remedy is not a smarter control on the same question but a second, more direct one. A z-extent read off the artefact itself beats an inference from one sampled face.
...
**IDENTITY NEEDS A CATEGORICAL CHANNEL, NOT A SCALAR ONE — added 2026‑08‑12, Patrick's ruling, and it now has THREE INSTANCES, the third in 3D.**
...
**Instance three — footprint, found at handoff 0016, the first instance in 3D rather than in plan.** `shower` (36×36), `walk_in_shower` (60×42) and `glass_shower` (60×48) are near-identical footprints at the **same height** (78″), so nothing about their extruded boxes tells them apart. **Their identity was being carried by size — a scalar — and it was never designed to carry it; nobody chose footprint as an identity channel, it simply is the only thing that differs between three otherwise-identical boxes.** Confirmed in a side-by-side render, which is exactly why a side-by-side is what exposes a scalar channel: **three items read as a set, not as three kinds.** The categorical channel that *is* available: the **internal region** — a glass enclosure has a door panel and a curb, a walk-in has a bench, a plain shower has neither. Different kinds of mark, not points on an axis. (Not yet fixed — see the enclosure-form entries below; the artwork brief is written, the redraw is not.)

> **The three instances together:** thickness failed because two types share a real thickness; fineness failed because two ladders at different pitches are one ladder at any zoom you actually work at; footprint fails because near-identical boxes at the same height are one box wearing different names. **Ask of any identity channel: are its values points on an axis, or different kinds of mark?** If the former, it will hold in a side-by-side comparison and fail at a glance — and a glance is what the user gives it.

**A CHECK REQUEST NAMES THE PLAN AND LISTS THE ITEMS IT CONTAINS — added 2026‑08‑15, Patrick's ruling, from the same handoff.**

**The instance it closes:** a check verdict — *"the boat trailer is chunky looking"* — was said about a render that did not contain `boat_trailer` at all. The subject was never in frame; the verdict was about *some other dark form in the scene*. **A verdict about a state the instrument never reached is vacuous by precondition, and a manual check is exactly as exposed to that failure as an automated test is** — the negative-assertion rule above, arriving at a person rather than at code.

**So: a check request states which plan was loaded and which items are in it, before the verdict is read.** The reviewer can then confirm the subject was in frame **as a precondition**, the same discipline every negative assertion in this project already owes. Two omissions compound the same failure and are named alongside it, because all three trace to one cause — a check that arrived by terminal screenshot rather than as a committed report: no plan named, no evidence PNG committed, and no `NNNN-report.md` at all (the channel contract's own requirement, missed the day after it was written). **A check request is a report.**
...
**A RECEIPT'S SENTENCE CAN CLAIM MORE THAN ITS METRIC MEASURED — a sibling of the rule above, found three times on one feature and named at handoff 0016.** The count is honest; the *sentence built around it* silently substitutes a stronger claim for the narrower one actually measured.

**All three instances are prism's, in order:** *"28 of 28 usable"* (2026‑08‑12) counted a threshold crossing, not usability — withdrawn when `lawnmower` and `snowblower`, structurally identical, landed on opposite sides of one 25% line. *"28 → 1"* (2026‑08‑13) counted **items that extrude**, and the sentence around it — *"a third of the catalog renders as a box"* — was read as *"renders correctly"*; the honest correction was **the outer outline is a plain rectangle for 17 of 18**, because *extrudes something* and *extrudes a recognisable body* are different predicates the count could not tell apart. *"Enclosure 6 of 7"* (2026‑08‑13, supporting the retirement in [`0015-ruling.md`](handoff/0015-ruling.md)) made the identical substitution for a different form, unnoticed until 0016 pointed a picture at it: it counted items that **extruded a body**, and *"extrudes a body"* is not *"reads as its kind"* — `shower` and `walk_in_shower` both extrude successfully to a box with no internal feature, so the model's own report (which names only items with *nothing* to extrude) cannot see them either.

**The general form: a count measures whatever predicate its instrument actually tests, and the prose beside it is where the stronger claim sneaks in.** Ask of any such receipt: *state the exact predicate counted, in the instrument's own terms, and compare it word for word against the sentence written beside the number.* Where they differ, the sentence is the thing to fix, not the count.
