...
| 19 | **`extract_from_reference` leaves its walls unwelded.** The PNG extractor writes detected walls straight into the scene and calls `_commit_if_changed` (`mainwindow.py:1644`) with no weld pass — and per the corrected **F5**, nothing welds them afterwards either, because load doesn't and coalesce doesn't. So every extracted plan is born with open junctions, which is precisely the condition that leaks room detection between spaces. Measured at P1.6: 2 unwelded ends on the `test_extract` fixture. Not raised by shadow mode (the 9″ tolerance is a gesture, not an invariant — see the register note below), so it needs fixing on purpose, not by a gate. | `mainwindow.py:1644` (`extract_from_reference`) | **P2.1** |
