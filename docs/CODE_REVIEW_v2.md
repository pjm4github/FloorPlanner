...
**Moved to [`defects/`](defects/)** on 2026-08-06 -- one record per file,
indexed at [`defects/INDEX.md`](defects/INDEX.md). The 50 rows' three cells
moved byte-for-byte; the field rules, the label taxonomy, the `rank` caveat and
the four standing notes that sat below the table are in
[`defects/README.md`](defects/README.md).

**The count changed and nothing was closed or removed** -- the categories got
honest: **38 defects**, 6 gaps, 1 limit, 5 tasks, across 50 records (ids 1-49
consecutive, plus `12a`). `defects/README.md` says who ruled what.
