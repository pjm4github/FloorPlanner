...
- Furnishing SVG viewBox is in inches and equals the item's real-world footprint; `manifest.json` carries `width_in`/`depth_in`. The app loads catalogs dynamically — adding a symbol needs no app-code change.
