...
- **Shared-wall model:** a `WallItem` carries a list `rooms` (the rooms it borders); on a boundary between two rooms there is **one** shared wall, not a duplicate per room. `RoomItem.bind_wall`/`unbind_wall` add/remove the association without stealing or deleting. **A CORNER IS ONE `Vertex` (Phase 3, merged at `03f3868`)** that the walls *and* the room outlines hold, so moving it moves everything on it — which is why there is no re-detection pass and no coalesce family any more. Overlapping collinear same-type walls **merge** (`merge_wall` / `merge_all`), ends **weld** onto a shared corner (`weld_wall_ends` / `weld_scene`), and Edit ▸ Coalesce all walls now is the explicit plan-wide pass (`normalize_walls` = merge + weld + split); `SETTINGS["auto_coalesce"]` still gates the automatic half. Grouped walls never merge (the `group() is None` guard). Deleting a wall is genuinely deletion since P4.1 (`delete_wall` — the fracture family is gone): the room survives via its stored outline and the vacated edge becomes an open edge. **Rooms are movable units since P4.2** (`extract.py`): `extract_room` lifts a room out of the network (`placed → floating`, I12 by construction — a wall the room doesn't wholly own is copy-trimmed to the room's own edge stretch and the plan keeps every wall it had) and `join_room` welds it back (split landings, weld at `vertex_weld_in`, merge coincident with opening dedup, rebind); a placed room's label-drag **is** extract→move→join through those ops (`_privatize_shared_walls` is gone), a floating room paints distinctly and moves as a closed unit, and the document walk folds each floating room in its own vertex namespace (I14 exempts floating-vs-plan pairs). **Still on the old shape, and dies in Phase 4:** grouping a room *alone* duplicates its walls into the group (`duplicate_wall`, → P4.5, and the source of defect 23's stranding).
